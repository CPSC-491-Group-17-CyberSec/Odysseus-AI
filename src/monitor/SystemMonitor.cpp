  ...
  void run() {
    ...
    if (cfg.processScanEnabled) {
      ...
      qInfo().noquote() << QString(
                               "[SysMon] enumerated %1 process(es) in %2 ms "
    ...
    if (cfg.suspiciousProcessHeuristicsEnabled && !snap.processes.isEmpty()) {
      ...
      snap.suspicious = SuspiciousHeuristics::evaluate(snap.processes, cfg.codeSigningEnabled);
      qInfo().noquote() << QString("[SysMon] heuristics flagged %1 process(es) in %2 ms")
    ...
    if (cfg.persistenceScanEnabled) {
      ...
      qInfo().noquote() << QString("[SysMon] persistence scan found %1 item(s) in %2 ms")
    ...
    if (snap.rootkit.ran && cfg.verboseLogging) {
      qDebug().noquote()
          << QString("[SysMon] rootkit pass took %1 ms").arg(t.elapsed() - persistMs);
    }

    qInfo().noquote() << QString(
                             "[SysMon] snapshot complete in %1 ms — "
...
bool SystemMonitor::refresh() {
  ...
  connect(m_worker, &SystemMonitorWorker::snapshotReady, this, [this](const SystemSnapshot& s) {
    m_lastSnapshot = s;
    emit snapshotReady(s);
  }, Qt::QueuedConnection);
  connect(m_worker, &SystemMonitorWorker::snapshotReady, m_thread, &QThread::quit);
