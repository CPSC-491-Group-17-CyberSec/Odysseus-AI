...
QString resolveUser(int uid) {
  ...
  const int rc = ::getpwuid_r(static_cast<uid_t>(uid), &pwd, buf, sizeof(buf), &result);
...
bool list(QVector<ProcessInfo>& out, int& restrictedCount) {
  ...
  if (::sysctl(mib, 3, nullptr, &sz, nullptr, 0) != 0) {
    qWarning() << "[SysMon] sysctl(KERN_PROC_ALL) sizing failed:" << std::strerror(errno);
    return false;
  }
  ...
  if (::sysctl(mib, 3, raw.data(), &sz, nullptr, 0) != 0) {
    qWarning() << "[SysMon] sysctl(KERN_PROC_ALL) read failed:" << std::strerror(errno);
    return false;
  }
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (kp.kp_proc.p_pid == 0)
      continue;  // skip kernel_task pid 0
  ...
  for (const QString& e : entries) {
    ...
    char linkBuf[4096] = {0};
    const ssize_t got =
        ::readlink(QFile::encodeName(base + "/exe").constData(), linkBuf, sizeof(linkBuf) - 1);
    if (got > 0) {
      linkBuf[got] = 0;
  ...
  if (snap == INVALID_HANDLE_VALUE) {
    qWarning() << "[SysMon] CreateToolhelp32Snapshot failed:" << ::GetLastError();
    return false;
  }
