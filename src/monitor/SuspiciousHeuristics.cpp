...
QString testPath(const ProcessInfo& p)
{
    ...
    // Hidden directory anywhere on the path
...
{
    if (s.length() < 5) return 0.0;     // too short to judge
    ...
    for (int i = 0; i < s.size(); ++i) {
        ...
        if (c.isLetter()) {
            ...
            if (QString("aeiouy").contains(lc)) ++vowels;
        }
        if (i > 0 && prev.isLetter() && c.isLetter()
            && (prev.isUpper() != c.isUpper()))
            ++caseFlips;
        prev = c;
    }
    ...
    double score = 0.0;
    if (digitRatio > 0.3)               score += 0.4;
    if (vowelRatio < 0.2 && letters > 4) score += 0.4;
    if (caseRatio > 0.4)                 score += 0.3;
...
{
    if (p.cmdLine.isEmpty() || p.cmdLine == "(restricted)") return {};
    ...
    static const QStringList badPatterns = {
        ...
        "-EncodedCommand",
        "base64 -d",
        "curl http", "curl https", "wget http",
        "/bin/sh -i",  "/bin/bash -i",
        "bash -c \"$(curl",
        "nc -e",
        "ncat -e",
...
QString testRootFromUserPath(const ProcessInfo& p)
{
    ...
    if (p.exePath.startsWith("/tmp/")
        || p.exePath.startsWith("/var/tmp/")
        || p.exePath.startsWith("/private/tmp/")
        || p.exePath.startsWith("/private/var/tmp/"))
                                     ...
                                     bool checkSigning)
{
    ...
    for (const ProcessInfo& p : processes) {
        if (p.isOurProcess) continue;     // never flag ourselves

        QStringList reasons;
        int score = 0;

        if (const QString r = testPath(p);             !r.isEmpty()) { reasons.append(r); score += 40; }
        if (const QString r = testExeMissing(p);        !r.isEmpty()) { reasons.append(r); score += 50; }
        if (const QString r = testRandomName(p);        !r.isEmpty()) { reasons.append(r); score += 15; }
        if (const QString r = testCmdLine(p);           !r.isEmpty()) { reasons.append(r); score += 30; }
        if (const QString r = testRootFromUserPath(p);  !r.isEmpty()) { reasons.append(r); score += 40; }
        ...
        if      (sp.score >= 60) sp.severity = "high";
        else if (sp.score >= 30) sp.severity = "medium";
        else                      sp.severity = "low";

        out.append(std::move(sp));
    }

    std::sort(out.begin(), out.end(),
              [](const SuspiciousProcess& a, const SuspiciousProcess& b) {
