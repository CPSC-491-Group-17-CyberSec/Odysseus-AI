...
ProcessControl makeDefaultProcessControl() {
  ...
  pc.kill = [](int64_t pid, int sig) -> int {
    if (pid <= 1)
      return EINVAL;  // never touch init/launchd
...
ResponseManager::ResponseManager()
    : ResponseManager(
          std::make_shared<Allowlist>(Allowlist::defaultPath()),
...
ResponseManager::ResponseManager(
    std::shared_ptr<Allowlist> allowlist,
...
ResponseConfig ResponseManager::config() const {
  return config_;
}
void ResponseManager::setConfig(const ResponseConfig& cfg) {
  config_ = cfg;
}

Allowlist& ResponseManager::allowlist() {
  return *allowlist_;
}
Quarantine& ResponseManager::quarantine() {
  return *quarantine_;
}
ActionLog& ResponseManager::actionLog() {
  return *actionLog_;
}
...
// ---------------------------------------------------------------------------
bool ResponseManager::isActionAvailable(
    ActionType action, const ActionTarget& target, std::string* why) const {
  ...
  switch (action) {
    ...
    case ActionType::CopyPath:
      return !target.path.empty() || deny("No path to copy.");

    case ActionType::CopyHash:
      return !target.sha256.empty() || deny("No SHA-256 available for this target.");

    case ActionType::CopyCommandLine:
      return !target.commandLine.empty() || deny("No command line captured.");
    ...
    case ActionType::RestoreFromQuarantine:
      return !target.sourceId.empty() || deny("Restore needs a quarantine entry id (sourceId).");
    ...
    case ActionType::KillProcess:
      ...
      if (isCriticalProcessName(target.processName))
        return deny(
            "This process is on the critical-process "
            "blocklist.");
      return true;
...
ActionResult ResponseManager::execute(const ActionRequest& req) {
  ...
  if (requiresConfirmation(req.action) && !req.userConfirmed) {
    res.errorMessage = "Confirmation required: this action is destructive or sensitive.";
    recordAudit(req, res);
    return res;
  }
...
ActionResult ResponseManager::executeCopyText(
    const ActionRequest& /*req*/, const std::string& text) {
  ActionResult res;
  if (text.empty()) {
  ...
  res.message = res.success ? "Copied to clipboard." : "Clipboard write failed.";
  if (!res.success)
    res.errorMessage = res.message;
  return res;
}
...
ActionResult ResponseManager::executeQuarantine(const ActionRequest& req) {
  ActionResult res;
  auto entry =
      quarantine_->quarantine(req.target.path, req.target.sha256, req.reason, req.target.sourceId);
  if (!entry) {
...
ActionResult ResponseManager::executeRestore(const ActionRequest& req) {
  ActionResult res = quarantine_->restore(req.target.sourceId, req.restorePolicy);
...
ActionResult ResponseManager::executeKillProcess(const ActionRequest& req) {
  ...
  if (stillAlive && req.allowSigkillEscalation) {
    ...
    if (rc != 0) {
      res.errorMessage = "SIGTERM sent; SIGKILL escalation failed: errno=" + std::to_string(rc);
      return res;
    }
  ...
  if (stillAlive) {
    res.message =
        "SIGTERM sent; process still running. Re-issue with "
...
ActionResult ResponseManager::executeAddToAllowlist(const ActionRequest& req) {
  ...
  } else if (req.target.kind == TargetKind::Process && !req.target.path.empty()) {
...
ActionResult ResponseManager::executeRemoveFromAllowlist(const ActionRequest& req) {
  ActionResult res;
  if (!config_.allowlistEnabled) {
    res.errorMessage = "Allowlist is disabled in settings.";
    return res;
  ...
  bool removed = tryRemove(AllowlistEntry::Kind::FileSha256, req.target.sha256) ||
...
// Audit logging.
// ---------------------------------------------------------------------------
void ResponseManager::recordAudit(const ActionRequest& req, const ActionResult& res) {
  if (!actionLog_)
    return;
  ActionLogRecord r;
  r.action = req.action;
...
std::vector<ActionResult> ResponseManager::executeBatch(
  ...
  for (const auto& r : reqs) {
    ...
    if (!out.back().success && !continueOnError)
      break;
  }
  return out;
}
