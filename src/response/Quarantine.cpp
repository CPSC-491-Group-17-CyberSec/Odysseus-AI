...
Quarantine::Quarantine()
    : Quarantine(defaultAppDataDir() + "/quarantine",
                 defaultAppDataDir() + "/quarantine_metadata.jsonl") {}
...
bool Quarantine::loadLocked() {
    entries_.clear();
    std::error_code ec;
    if (!fs::exists(metadataPath_, ec)) return true;
    ...
    while (std::getline(in, line)) {
        ...
        if (!e.id.empty()) entries_.push_back(std::move(e));
    }
    return true;
}
...
std::optional<QuarantineEntry> Quarantine::quarantine(
    ...
    if (ec) entry.originalPath = originalPath;
    entry.sha256         = sha256;
    ...
    entry.quarantinePath = (fs::path(quarantineDir_) /
                            (baseName + "." + entry.id + ".quarantine"))
                               .string();
    ...
    if (ec) {
        ec.clear();
        fs::copy_file(entry.originalPath, entry.quarantinePath,
    ...
    fs::permissions(entry.quarantinePath,
                    fs::perms::owner_read,
                    fs::perm_options::replace, ec);

    entries_.push_back(entry);
    if (!flushLocked()) return std::nullopt;
    return entry;
}
...
// Restore.
// ---------------------------------------------------------------------------
ActionResult Quarantine::restore(const std::string& entryId,
    ...
    if (!fs::exists(it->quarantinePath, ec)) {
        res.errorMessage = "restore: quarantined file missing on disk: "
                         + it->quarantinePath;
        return res;
    }
    ...
    if (destinationExists) {
        switch (policy) {
            case RestoreConflictPolicy::AskUser:
                ...
                res.message = "Original path already exists. Choose how to "
                              "restore.";
                return res;
            ...
            case RestoreConflictPolicy::Overwrite:
                ...
                if (ec) {
                    res.errorMessage = "restore overwrite: could not remove "
                                       "existing file: " + ec.message();
                    return res;
                }
                break;
            case RestoreConflictPolicy::RestoreWithNewName: {
                ...
                fs::path candidate;
                do {
                    candidate = p.parent_path() /
                                (stem + ".restored-" + std::to_string(n++) + ext);
    ...
    if (ec) {
        ec.clear();
        fs::copy_file(it->quarantinePath, destination,
                      fs::copy_options::overwrite_existing, ec);
        if (ec) {
...
std::optional<QuarantineEntry> Quarantine::findById(
...
std::optional<QuarantineEntry> Quarantine::findByOriginalPath(
    ...
    std::lock_guard<std::mutex> g(mutex_);
    for (const auto& e : entries_) {
        if (e.originalPath == originalPath) return e;
    }
    return std::nullopt;
}
