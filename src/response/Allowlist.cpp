...
const char* kindToString(AllowlistEntry::Kind k) { return toString(k); }

std::optional<AllowlistEntry::Kind> parseKind(const std::string& s) {
    if (s == "FileSha256")        return AllowlistEntry::Kind::FileSha256;
    if (s == "FilePath")          return AllowlistEntry::Kind::FilePath;
    if (s == "ProcessPath")       return AllowlistEntry::Kind::ProcessPath;
    if (s == "PersistenceLabel")  return AllowlistEntry::Kind::PersistenceLabel;
    if (s == "PersistencePath")   return AllowlistEntry::Kind::PersistencePath;
    if (s == "AlertSignatureKey") return AllowlistEntry::Kind::AlertSignatureKey;
    return std::nullopt;
}
...
Allowlist::Allowlist(std::string filePath) : filePath_(std::move(filePath)) {
...
bool Allowlist::loadLocked() {
    ...
    while (std::getline(in, line)) {
        ...
        if (!obj.ok) continue;  // skip malformed lines defensively

        auto kindStr = obj.getString("kind");
        auto value   = obj.getString("value");
        if (!kindStr || !value) continue;
...
bool Allowlist::add(AllowlistEntry entry) {
    ...
    if (entry.addedEpoch == 0) entry.addedEpoch = nowEpoch();
    ...
    entries_.erase(
        std::remove_if(entries_.begin(), entries_.end(),
                       [&](const AllowlistEntry& e) {
                           return e.kind == kind && e.value == value;
                       }),
        entries_.end());
...
// Queries.
// ---------------------------------------------------------------------------
bool Allowlist::contains(AllowlistEntry::Kind kind,
                         const std::string& value) const {
    std::lock_guard<std::mutex> g(mutex_);
    for (const auto& e : entries_) {
        if (e.kind == kind && e.value == value) return true;
    }
    return false;
}

bool Allowlist::isFileIgnored(const std::string& path,
                              const std::string& sha256) const {
    std::lock_guard<std::mutex> g(mutex_);
    for (const auto& e : entries_) {
        if (e.kind == AllowlistEntry::Kind::FilePath   && !path.empty()
            && e.value == path)   return true;
        if (e.kind == AllowlistEntry::Kind::FileSha256 && !sha256.empty()
            && e.value == sha256) return true;
    }
    return false;
}
...
bool Allowlist::isProcessIgnored(const std::string& processPath) const {
    ...
    std::lock_guard<std::mutex> g(mutex_);
    for (const auto& e : entries_) {
        if (e.kind == AllowlistEntry::Kind::ProcessPath
            && e.value == processPath) return true;
    }
    return false;
}

bool Allowlist::isPersistenceIgnored(const std::string& label,
                                     const std::string& path) const {
    std::lock_guard<std::mutex> g(mutex_);
    for (const auto& e : entries_) {
        if (e.kind == AllowlistEntry::Kind::PersistenceLabel && !label.empty()
            && e.value == label) return true;
        if (e.kind == AllowlistEntry::Kind::PersistencePath  && !path.empty()
            && e.value == path)  return true;
    }
    return false;
}
...
bool Allowlist::isAlertSignatureIgnored(const std::string& signatureKey) const {
    if (signatureKey.empty()) return false;
    std::lock_guard<std::mutex> g(mutex_);
    for (const auto& e : entries_) {
        if (e.kind == AllowlistEntry::Kind::AlertSignatureKey
            && e.value == signatureKey) return true;
    }
    return false;
}
