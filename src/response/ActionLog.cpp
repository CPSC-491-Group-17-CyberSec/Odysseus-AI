...
std::optional<TargetKind> parseTargetKind(const std::string& s) {
  ...
  if (s == "Process")
    return TargetKind::Process;
  if (s == "Persistence")
    return TargetKind::Persistence;
  if (s == "Integrity")
    return TargetKind::Integrity;
  if (s == "KernelExtension")
    return TargetKind::KernelExtension;
  if (s == "Unknown")
...
std::string summarizeTarget(const ActionTarget& t) {
  ...
  switch (t.kind) {
    case TargetKind::File:
      ...
      if (!t.sha256.empty())
        os << " sha256=" << t.sha256.substr(0, 12);
      break;
    ...
    case TargetKind::Persistence:
      os << "persistence: " << (t.label.empty() ? t.path : t.label);
      break;
    case TargetKind::Integrity:
      os << "integrity: " << (t.path.empty() ? t.sourceId : t.path);
      break;
...
ActionLog::ActionLog(std::string filePath)
...
std::string ActionLog::append(ActionLogRecord record) {
  ...
  if (record.id.empty())
    record.id = makeId();
  if (record.timestampEpoch == 0)
    record.timestampEpoch = nowEpoch();
...
std::vector<ActionLogRecord> ActionLog::readAll() const {
  ...
  while (std::getline(in, line)) {
    ...
    r.action = parseActionType(obj.getString("action").value_or("None")).value_or(ActionType::None);
    r.targetKind = parseTargetKind(obj.getString("targetKind").value_or("Unknown"))
