...
bool execSql(sqlite3* db, const char* sql, QString* errOut = nullptr) {
  ...
  if (rc != SQLITE_OK) {
    QString msg = QString("SQLite error: %1  [SQL: %2]").arg(errmsg ? errmsg : "unknown").arg(sql);
    if (errOut)
      *errOut = msg;
    else
      qWarning() << msg;
...
bool ScanDatabase::openDatabase(sqlite3** db, const QString& path) const {
  ...
  int rc = sqlite3_open_v2(
      ...
      nullptr);
  if (rc != SQLITE_OK) {
    qWarning() << "ScanDatabase: sqlite3_open_v2 failed for" << path << "-" << sqlite3_errmsg(*db);
    sqlite3_close(*db);
    *db = nullptr;
    return false;
  }
...
void ScanDatabase::saveScanRecord(const ScanRecord& record) {
  ...
  enqueueWrite([copy, this](sqlite3* db) {
    ...
    if (sqlite3_prepare_v2(db, scanSql, -1, &stmt, nullptr) != SQLITE_OK) {
      qWarning() << "saveScanRecord: prepare scan insert failed:" << sqlite3_errmsg(db);
      execSql(db, "ROLLBACK;");
      return;
    }
    ...
    if (sqlite3_step(stmt) != SQLITE_DONE) {
      qWarning() << "saveScanRecord: step scan insert failed:" << sqlite3_errmsg(db);
    ...
    for (const SuspiciousFile& sf : copy.findings) {
      ...
      if (sqlite3_prepare_v2(db, findingSql, -1, &fstmt, nullptr) != SQLITE_OK) {
        qWarning() << "saveScanRecord: prepare finding insert failed:" << sqlite3_errmsg(db);
        continue;
      }
      ...
      if (sqlite3_step(fstmt) != SQLITE_DONE) {
        qWarning() << "saveScanRecord: step finding insert failed:" << sqlite3_errmsg(db);
      }
    ...
    QMetaObject::invokeMethod(
        this, [this, scanId]() { emit recordSaved(scanId); }, Qt::QueuedConnection);
  });
}
...
QVector<ScanRecord> ScanDatabase::loadAllScanRecords() const {
  ...
  while (sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    QString tsStr = QString::fromUtf8(reinterpret_cast<const char*>(sqlite3_column_text(stmt, 1)));
...
QVector<ScanRecord> ScanDatabase::loadRecentScanHeaders(int n) const {
  ...
  while (sqlite3_step(stmt) == SQLITE_ROW) {
    QString ts = QString::fromUtf8(reinterpret_cast<const char*>(sqlite3_column_text(stmt, 0)));
...
QHash<QString, CacheEntry> ScanDatabase::loadScanCache() const {
  ...
  while (sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    e.lastModifiedMs = QDateTime::fromString(e.lastModified, Qt::ISODate).toMSecsSinceEpoch();
    ...
    const bool versionsMatch = (curModel.isEmpty() || rowModel == curModel) &&
  ...
  qDebug().noquote() << "loadScanCache: loaded" << cache.size() << "entries"
...
void ScanDatabase::flushScanCache(const QVector<CacheEntry>& entries) {
  ...
  enqueueWrite([copy, now, modelV, rulesV, configV](sqlite3* db) {
    ...
    for (const CacheEntry& e : copy) {
      ...
      bindText(stmt, 5, e.isFlagged ? QStringLiteral("flagged") : QStringLiteral("clean"));
...
bool ScanDatabase::clearAllData() {
  ...
  if (sqlite3_open_v2(m_dbPath.toUtf8().constData(), &db, SQLITE_OPEN_READWRITE, nullptr) !=
      SQLITE_OK) {
