...
QString computeRulesVersion() {
  ...
  QDirIterator it(rulesDir, {"*.yar", "*.yara"}, QDir::Files, QDirIterator::Subdirectories);
  int count = 0;
  QDateTime latest;
  while (it.hasNext()) {
    it.next();
    ...
    if (!latest.isValid() || mt > latest)
      latest = mt;
    ++count;
  }
  if (count == 0)
    return {};
  return QString("%1:%2").arg(count).arg(latest.toString(Qt::ISODate));
}

QString computeConfigHash() {
  const ScannerConfig& cfg = ScannerConfigStore::current();
  const QByteArray bytes = QJsonDocument(cfg.toJson()).toJson(QJsonDocument::Compact);
