...
bool EmberDetector::load(const std::string& modelPath, const std::string& scalerPath) {
  ...
  // ── Load scaler ────────────────────────────────────────────────────
  {
    ...
    if (static_cast<int>(nFeatures) != EMBER_FEATURES) {
      std::cerr << "[EmberDetector] Scaler feature count mismatch: " << nFeatures << " vs "
                << EMBER_FEATURES << std::endl;
      return false;
    }

    m_impl->mean.resize(EMBER_FEATURES);
    m_impl->scale.resize(EMBER_FEATURES);
    f.read(reinterpret_cast<char*>(m_impl->mean.data()), EMBER_FEATURES * sizeof(double));
    f.read(reinterpret_cast<char*>(m_impl->scale.data()), EMBER_FEATURES * sizeof(double));
  ...
  {
    int numIterations = 0;
    int ret = LGBM_BoosterCreateFromModelfile(modelPath.c_str(), &numIterations, &m_impl->booster);

    if (ret != 0) {
      std::cerr << "[EmberDetector] LightGBM load failed: " << LGBM_GetLastError() << std::endl;
      return false;
    }

    std::cout << "[EmberDetector] LightGBM model loaded: " << modelPath << " (" << numIterations
              << " iterations)" << std::endl;
  }
...
float EmberDetector::score(const std::vector<float>& rawFeatures) const {
  ...
  for (int i = 0; i < EMBER_FEATURES; ++i) {
    ...
    scaled[i] = (s > 1e-10) ? (static_cast<double>(rawFeatures[i]) - m_impl->mean[i]) / s : 0.0;
  }
  ...
  if (ret != 0) {
    std::cerr << "[EmberDetector] Prediction error: " << LGBM_GetLastError() << std::endl;
    return -1.0f;
  }
