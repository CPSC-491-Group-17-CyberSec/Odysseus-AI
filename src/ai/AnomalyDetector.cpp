...
bool AnomalyDetector::loadModel(const std::string& onnxPath) {
  ...
  try {
    ...
    m_impl->session =
        std::make_unique<Ort::Session>(m_impl->env, widePath.c_str(), m_impl->sessionOpts);
#else
    m_impl->session =
        std::make_unique<Ort::Session>(m_impl->env, onnxPath.c_str(), m_impl->sessionOpts);
#endif
    ...
    m_impl->loaded = true;
    std::cout << "[AnomalyDetector] Model loaded: " << onnxPath << " (" << numInputs << " inputs, "
              << numOutputs << " outputs, " << m_impl->featureCount << " features)" << std::endl;
    return true;
  ...
  try {
    ...
    if (outputTensors.size() >= 2) {
      ...
      if (numProbs >= 2) {
        ...
        if (diagEnabled) {
          std::cout << "[DIAG:ONNX] p(benign)=" << std::fixed << std::setprecision(6) << pBenign
                    << "  p(malicious)=" << pMalicious
                    << "  RETURNING=" << std::clamp(pMalicious, 0.0f, 1.0f) << std::endl;
        }
        return std::clamp(pMalicious, 0.0f, 1.0f);
      } else if (numProbs == 1) {
        if (diagEnabled) {
          std::cout << "[DIAG:ONNX] single-prob output=" << std::fixed << std::setprecision(6)
                    << probs[0] << std::endl;
        }
    ...
    if (!outputTensors.empty()) {
      ...
      if (diagEnabled) {
        std::cout << "[DIAG:ONNX] fallback single-tensor output=" << std::fixed
                  << std::setprecision(6) << data[0] << std::endl;
      }
