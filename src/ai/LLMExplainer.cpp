...
std::string queryOllamaImpl(const LLMExplainer::Config& cfg, const std::string& prompt) {
  ...
  if (reply->error() != QNetworkReply::NoError) {
    std::string err = "[LLM Explainer] Ollama error: " + reply->errorString().toStdString();
    reply->deleteLater();
    return err;
  }
...
LLMExplainer::LLMExplainer(const Config& config)
    : m_config(config) {}

LLMExplainer::~LLMExplainer() = default;

void LLMExplainer::setConfig(const Config& config) {
  m_config = config;
}
...
bool LLMExplainer::isAvailable() const {
  QNetworkAccessManager mgr;
  QNetworkRequest req(QUrl(QString::fromStdString("http://localhost:11434/api/tags")));
  req.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
...
// ============================================================================
std::string LLMExplainer::buildPrompt(
  ...
  // ── File metadata ───────────────────────────────────────────────────
  ss << "FILE: " << fi.fileName().toStdString() << " (." << fi.suffix().toStdString() << ")\n"
  ...
  if (static_cast<int>(features.size()) == kFeatureCount) {
    ...
    if (features[7] > 0.3f)
      ss << "  High-byte ratio: " << std::setprecision(1) << (features[7] * 100)
         << "% [ELEVATED]\n";
    ...
    if (features[16] > 0.5f) {
      ...
      if (features[18] > 0.875f)
        ss << ", max section entropy VERY HIGH";
      if (features[20] < 0.5f)
        ss << ", entry point OUTSIDE code section";
      if (features[21] < 0.5f)
        ss << ", NO debug info";
      if (features[25] > 0.5f)
        ss << ", anomalous section names";
      if (features[27] > 0.3f)
        ss << ", inflated virtual size";
      ss << "\n";
    }
...
// ============================================================================
std::string LLMExplainer::explain(
    const std::string& filePath,
    const std::vector<float>& features,
    float anomalyScore,
    const std::string& classificationLevel) const {
  std::string prompt = buildPrompt(filePath, features, anomalyScore, classificationLevel);
  ...
  if (response.empty()) {
    return "[LLM Explainer] No response received from Ollama. "
           ...
           m_config.model + " model is pulled (ollama pull " + m_config.model + ").";
  }
...
// queryOllama() carried.
// ============================================================================
void LLMExplainer::explainAsync(
    const std::string& filePath,
    const std::vector<float>& features,
    float anomalyScore,
  ...
  QThreadPool::globalInstance()->start([prompt = std::move(prompt),
                                        configCopy = std::move(configCopy),
                                        cb = std::move(callback)]() mutable {
    std::string response = queryOllamaImpl(configCopy, prompt);

    bool success = !response.empty() && response.find("[LLM Explainer]") == std::string::npos;
