...
static std::vector<uint8_t> readFileBytes(const std::string& path)
{
    ...
    if (size <= 0 || size > 200LL * 1024 * 1024) return {};
                                  ...
                                  std::vector<float>& out)
{
    ...
    static const std::set<std::string> scriptExts = {
        "bat", "cmd", "ps1", "vbs", "js", "wsh", "wsf", "py", "sh",
        "bash", "pl", "rb", "php", "hta"
    };
    static const std::set<std::string> dllExts = {
                                   ...
                                   std::vector<float>& out)
{
    ...
    if (len == 0) return;  // features stay 0.0f
    ...
    for (int b = 0x01; b <= 0x1F; ++b) {
        if (b == 0x09 || b == 0x0A || b == 0x0D) continue;  // skip tab, LF, CR
        ctrl += freq[b];
    }
    ...
    for (size_t i = 0; i < len; ++i) {
        if (fileBytes[i] == 0x00) {
            ++curNullRun;
            if (curNullRun > maxNullRun) maxNullRun = curNullRun;
        } else {
            curNullRun = 0;
        }
    }
                           ...
                           std::vector<float>& out)
{
    ...
    // Check MZ signature
    if (d[0] != 'M' || d[1] != 'Z') return;

    // e_lfanew at offset 0x3C
    uint32_t peOffset = readU32(d + 0x3C);
    if (peOffset + 24 > len) return;

    // Check PE\0\0 signature
    if (d[peOffset] != 'P' || d[peOffset+1] != 'E' ||
        d[peOffset+2] != 0  || d[peOffset+3] != 0)
        return;
    ...
    if (static_cast<size_t>(opt - d) + 2 > len) return;
    ...
    for (int s = 0; s < numSections; ++s) {
        ...
        if (rawOffset > 0 && rawSize > 0 && rawOffset + rawSize <= len) {
            ...
            if (se > maxSectEntropy) maxSectEntropy = se;
        }
        ...
        if (isCode) {
            ...
            if (entryPointRVA >= virtualAddr &&
                entryPointRVA < virtualAddr + virtualSize)
                epInCode = true;
        }
        ...
        if (rawSize > 0) {
            ...
            if (ratio > maxVirtRawRatio) maxVirtRawRatio = ratio;
        }
    }
    ...
    if (timeDateStamp > 0) {
        ...
        out[26] = (timeDateStamp < 631152000 || timeDateStamp > 1893456000)
                  ? 1.0f : 0.0f;
    }
...
static std::vector<std::string> extractStrings(const uint8_t* data, size_t len,
                                                int minLen = 4)
{
                              ...
                              std::vector<float>& out)
{
    ...
    for (const auto& s : strings) totalStrBytes += s.size();
    ...
    static const std::vector<std::string> suspiciousKeywords = {
        ...
        "FindWindow", "EnumProcesses", "Process32First", "CreateToolhelp32Snapshot"
    };
    ...
    for (const auto& s : strings) {
        ...
        // 35: registry paths
        if (s.find("HKEY_") != std::string::npos ||
            s.find("HKLM\\") != std::string::npos ||
            s.find("HKCU\\") != std::string::npos ||
        ...
        if (s.size() > 20) {
            ...
            if (static_cast<double>(alphaCount) / s.size() > 0.90 &&
                s.size() > 40)
                ++b64Count;
        }
    }
