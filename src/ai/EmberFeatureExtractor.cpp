...
// ============================================================================
static void extractByteHistogram(const std::vector<uint8_t>& bytes,
                                  std::vector<float>& out, int offset)
{
...
// ============================================================================
static void extractByteEntropyHistogram(const std::vector<uint8_t>& bytes,
                                         std::vector<float>& out, int offset)
{
    if (bytes.size() < 16) {
...
//   registry, MZ_headers.
// ============================================================================
static void extractStringFeatures(const std::vector<uint8_t>& bytes,
                                   std::vector<float>& out, int offset)
{
...
//   resources, etc.
// ============================================================================
static void extractGeneralFileInfo(const std::vector<uint8_t>& bytes,
                                    std::vector<float>& out, int offset)
{
...
// ============================================================================
static void extractHeaderFileInfo(const std::vector<uint8_t>& bytes,
                                   std::vector<float>& out, int offset)
{
    uint32_t peOffset = readLE<uint32_t>(bytes, 0x3C);
    if (peOffset + 24 >= bytes.size()) return;
    ...
    out[offset + 19] = static_cast<float>(readLE<uint16_t>(bytes, alignBase + 12));  // MajorImageVersion
    out[offset + 20] = static_cast<float>(readLE<uint16_t>(bytes, alignBase + 14));  // MinorImageVersion
    out[offset + 21] = static_cast<float>(readLE<uint16_t>(bytes, alignBase + 16));  // MajorSubsystemVersion
    out[offset + 22] = static_cast<float>(readLE<uint16_t>(bytes, alignBase + 18));  // MinorSubsystemVersion
    ...
    if (isPE32Plus) {
        ...
        out[offset + 48] = static_cast<float>(static_cast<double>(readLE<uint64_t>(bytes, stackBase + 8)));
        out[offset + 49] = static_cast<float>(static_cast<double>(readLE<uint64_t>(bytes, stackBase + 16)));
        out[offset + 50] = static_cast<float>(static_cast<double>(readLE<uint64_t>(bytes, stackBase + 24)));
    } else {
    ...
    out[offset + 55] = static_cast<float>(readLE<uint32_t>(bytes, alignBase + 20)); // Win32VersionValue
    out[offset + 56] = static_cast<float>(readLE<uint32_t>(bytes, optBase + 24));   // BaseOfData (PE32 only)
    out[offset + 57] = 0.0f;
    out[offset + 58] = 0.0f;
    out[offset + 59] = 0.0f;
    out[offset + 60] = 0.0f;
...
// ============================================================================
static void extractSectionInfo(const std::vector<uint8_t>& bytes,
                                std::vector<float>& out, int offset)
{
    uint32_t peOffset = readLE<uint32_t>(bytes, 0x3C);
    uint16_t numSections = readLE<uint16_t>(bytes, peOffset + 6);
    ...
    for (int i = 0; i < actualSections; ++i) {
        ...
        if (secOff + SECTION_SIZE > bytes.size()) break;
        ...
        out[offset + 200 + bucket] += static_cast<float>(characteristics >> 20);  // high bits most interesting
    }
}
...
// ============================================================================
static void extractImportsInfo(const std::vector<uint8_t>& bytes,
                                std::vector<float>& out, int offset)
{
    uint32_t peOffset = readLE<uint32_t>(bytes, 0x3C);
    uint16_t magic = readLE<uint16_t>(bytes, peOffset + 24);
    ...
    auto rvaToOffset = [&](uint32_t rva) -> size_t {
        for (int i = 0; i < numSections; ++i) {
            ...
            if (secSize == 0) secSize = readLE<uint32_t>(bytes, secOff + 8);
    ...
    if (importFileOff == 0 || importFileOff >= bytes.size()) return;
    ...
    for (int desc = 0; desc < 256; ++desc) {  // cap at 256 DLLs
        ...
        for (int fn = 0; fn < 4096; ++fn) {  // cap at 4096 functions per DLL
            ...
            if (byOrdinal) {
                ...
                if (hintOff == 0 || hintOff + 2 >= bytes.size()) continue;

                // Skip 2-byte hint, read name
                std::string funcName;
                for (size_t j = hintOff + 2; j < bytes.size() && bytes[j] != 0 && funcName.size() < 256; ++j)
...
// ============================================================================
static void extractExportsInfo(const std::vector<uint8_t>& bytes,
                                std::vector<float>& out, int offset)
{
    uint32_t peOffset = readLE<uint32_t>(bytes, 0x3C);
    uint16_t magic = readLE<uint16_t>(bytes, peOffset + 24);
    ...
    if (exportOff == 0 || exportOff + 40 > bytes.size()) return;
    ...
    for (int i = 0; i < actualNames; ++i) {
        ...
        if (nameRVAOff + 4 > bytes.size()) break;
...
// ============================================================================
static void extractDataDirectories(const std::vector<uint8_t>& bytes,
                                    std::vector<float>& out, int offset)
{
    uint32_t peOffset = readLE<uint32_t>(bytes, 0x3C);
    uint16_t magic = readLE<uint16_t>(bytes, peOffset + 24);
...
{
    if (fileBytes.size() < 64) return false;
    uint16_t mz = readLE<uint16_t>(fileBytes, 0);
    if (mz != MZ_MAGIC) return false;

    uint32_t peOffset = readLE<uint32_t>(fileBytes, 0x3C);
    if (peOffset + 4 > fileBytes.size()) return false;
...
std::vector<float> extractEmberFeatures(const std::string& filePath)
{
    ...
    if (size <= 0 || size > 200 * 1024 * 1024) return {};
