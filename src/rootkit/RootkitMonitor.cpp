...
void scan(const QVector<ProcessInfo>& existingProcesses, RootkitSnapshot& snap) {
  ...
  if (cfg.processCrossViewCheckEnabled) {
    ProcessCrossView::diff(
        existingProcesses, snap.crossView, snap.processSysctlCount, snap.processPsCount);
  } else {
  ...
  if (cfg.kernelExtensionCheckEnabled) {
    ...
    if (cfg.verboseLogging) {
      qDebug().noquote() << QString("[Rootkit] kernel extension scan took %1 ms")
  ...
  if (cfg.integrityCheckEnabled) {
    ...
    if (cfg.verboseLogging) {
      qDebug().noquote()
          << QString("[Rootkit] integrity verify took %1 ms").arg(t.elapsed() - afterKext);
    }
  } else {
