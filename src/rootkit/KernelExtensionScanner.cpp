...
//   *        *       1234ABCDE  com.example.endpoint (1.2.3/100) Example Endpoint  [activated enabled]
//
// kmutil showloaded:
...
void rateExtension(KernelExtension& k)
{
    ...
    for (const QString& n : k.notes) {
        if (n.contains("unsigned", Qt::CaseInsensitive)
            || n.contains("ad hoc",  Qt::CaseInsensitive))
            unsignedHint = true;
    }
...
static void parseSystemExtensions(const QString& output,
                                   QVector<KernelExtension>& out)
{
    int found = 0;
    ...
    while (it.hasNext()) {
        ...
        k.isApple       = (k.teamId == "APPLE" || k.teamId == "0000000000")
                         || looksLikeAppleBundle(k.bundleId);
        rateExtension(k);
        out.append(std::move(k));
        ++found;
    }
    qInfo().noquote()
        << QString("[Rootkit] system_extension(s) parsed: %1").arg(found);
}

// kmutil showloaded parser.
// We only need the bundle name field — the rest is bookkeeping.
static void parseKmutilShowloaded(const QString& output,
                                   QVector<KernelExtension>& out)
{
    int found = 0;
    int skipped = 0;
    ...
    for (const QString& line : output.split('\n')) {
        ...
        if (trimmed.startsWith("Executing:")) continue;
        ...
        if (!sawHeader) continue;       // ignore preamble
        ...
        if (!m.hasMatch()) { ++skipped; continue; }
    ...
    qInfo().noquote()
        << QString("[Rootkit] legacy_kext(s) parsed: %1 (skipped %2 unparseable line(s))")
              .arg(found).arg(skipped);
}
...
bool list(QVector<KernelExtension>& out, int& totalsOut)
{
    ...
    {
        const ToolResult t = runTool("/usr/bin/systemextensionsctl",
                                      { "list" });
        if (t.timedOut) {
            qWarning() << "[Rootkit] systemextensionsctl timed out — skipping system extensions";
        } else if (t.rc != 0) {
            qWarning().noquote()
                << "[Rootkit] systemextensionsctl returned" << t.rc
    ...
    // ── Legacy kexts (kmutil) ──────────────────────────────────────────
    {
        const ToolResult t = runTool("/usr/bin/kmutil",
                                      { "showloaded" });
        if (t.timedOut) {
            qWarning() << "[Rootkit] kmutil showloaded timed out — skipping legacy kexts";
        } else if (t.rc != 0) {
            qWarning().noquote()
                << "[Rootkit] kmutil showloaded returned" << t.rc
...
bool list(QVector<KernelExtension>& out, int& totalsOut)
{
    ...
    for (const QString& line : text.split('\n')) {
        ...
        rateExtension(k);
        if (k.severity == "low") k.severity = "medium";
        out.append(std::move(k));
        ++n;
    }
    totalsOut = n;
    qInfo().noquote()
        << QString("[Rootkit] linux_module(s): %1").arg(n);
    return true;
}
