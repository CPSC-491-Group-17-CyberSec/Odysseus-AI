...
QString readMacosVersion() {
  ...
  if (f.open(QIODevice::ReadOnly)) {
    ...
    for (const QString& line : text.split('\n')) {
      if (line.startsWith("VERSION_ID="))
        return line.mid(QString("VERSION_ID=").size()).remove('"').trimmed();
    }
  }
  return {};
#elif defined(Q_OS_WIN)
  ...
  // rebase granularity).
  return QString("%1 (build %2)").arg(QSysInfo::productVersion(), QSysInfo::kernelVersion());
#else
...
Baseline loadBaseline() {
  ...
  if (!f.open(QIODevice::ReadOnly)) {
    qWarning().noquote() << "[Integrity] cannot read baseline" << f.fileName() << "("
  ...
  if (err.error != QJsonParseError::NoError || !doc.isObject()) {
    qWarning().noquote() << "[Integrity] baseline JSON corrupt (" << err.errorString()
  ...
  b.baselinedAt = QDateTime::fromString(root.value("baselined_at").toString(), Qt::ISODate);
...
bool saveBaseline(const Baseline& b) {
  ...
  if (!f.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
    qWarning().noquote() << "[Integrity] cannot write baseline:" << f.errorString();
    return false;
  }
...
bool verify(
  ...
  const bool osChanged = baseline.existsOnDisk && !baseline.macosVersion.isEmpty() &&
                         !currentVersion.isEmpty() && baseline.macosVersion != currentVersion;

  // OS version changed → silently rebase. We still hash everything; we
  // just won't flag mismatches. Drop all old entries.
  if (osChanged) {
    qInfo().noquote() << QString(
                             "[Integrity] macOS version changed (%1 → %2) — "
                             "rebasing integrity baseline silently")
                             .arg(baseline.macosVersion)
                             .arg(currentVersion);
  ...
  if (firstRun) {
    qInfo().noquote() << "[Integrity] no baseline on disk — capturing first-run baseline";
  ...
  for (const QString& path : criticalPaths()) {
    ...
    if (it == baseline.entries.constEnd()) {
      ...
      f.status = "new";
      f.severity = "low";
      f.note = firstRun ? "Initial baseline entry" : "New file added to baseline";
      BaselineEntry e;
      e.sha256 = hex;
      e.size = sz;
      e.firstSeen = nowIso;
    ...
    f.severity = "high";
    f.note = QString(
        "SHA-256 differs from baseline. Possible tampering, "
  ...
  qInfo().noquote() << QString("[Integrity] checked=%1 ok=%2 new=%3 missing=%4 mismatches=%5%6%7")
                           .arg(checkedOut)
                           .arg(okCount)
                           .arg(newCount)
                           .arg(missingCount)
...
int forceRebase() {
  ...
  qInfo().noquote()
      << QString("[Integrity] force-rebased baseline with %1 entries").arg(updated.entries.size());
