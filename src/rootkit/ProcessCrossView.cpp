...
bool diff(
  ...
  if (t.timedOut || t.rc != 0 || t.out.isEmpty()) {
    qWarning().noquote() << "[Rootkit] ps -axo failed (rc=" << t.rc
  ...
  for (auto it = sysctlMap.constBegin(); it != sysctlMap.constEnd(); ++it) {
    if (!psMap.contains(it.key())) {
      ...
      f.reason = QString(
                     "PID %1 (%2) appears in sysctl but is missing "
  ...
  for (auto it = psMap.constBegin(); it != psMap.constEnd(); ++it) {
    if (!sysctlMap.contains(it.key())) {
      ...
      f.visibleIn = "ps-only";
      f.reason = QString(
                     "PID %1 (%2) appears in `ps` but is missing "
  ...
  qInfo().noquote() << QString(
                           "[Rootkit] cross-view: %1 sysctl PIDs vs %2 ps PIDs — "
                           "%3 disagreement(s) [%4]")
                           .arg(sysctlCountOut)
                           .arg(psCountOut)
                           .arg(findings.size())
