...
MonitoringService::MonitoringService(SystemMonitor* sysmon, QObject* parent)
    : QObject(parent), m_sysmon(sysmon)
{
    ...
    connect(m_timer, &QTimer::timeout,
            this,     &MonitoringService::onTimerFired);

    if (m_sysmon) {
        connect(m_sysmon, &SystemMonitor::snapshotReady,
                this,      &MonitoringService::onSnapshotReady,
                Qt::QueuedConnection);
    }
}
...
void MonitoringService::start()
{
    ...
    m_tickPending  = false;
    qInfo().noquote()
        << QString("[EDR] monitoring enabled interval=%1s").arg(sec);
...
void MonitoringService::reloadConfig()
{
    ...
    if (m_running && m_timer) {
        ...
        if (m_timer->interval() != sec * 1000) {
            ...
            qInfo().noquote()
                << QString("[EDR] interval updated to %1s").arg(sec);
        }
    }
}
...
void MonitoringService::onSnapshotReady(const SystemSnapshot& snap)
{
    ...
    for (const EDR::Alert& a : dr.newAlerts) {
        ...
        if (m_alerts.size() > kAlertHistoryCap)
            m_alerts.resize(kAlertHistoryCap);
        qInfo().noquote()
            << QString("[EDR] alert %1 %2: %3 (key=%4)")
                  .arg(EDR::severityToText(fresh.severity).toUpper(),
                       fresh.category, fresh.title, fresh.dedupKey);
    ...
    for (auto it = dr.currentKeys.constBegin();
              it != dr.currentKeys.constEnd(); ++it) {
        if (!m_active.contains(it.key())) continue;
        ...
        for (const EDR::Alert& a : dr.newAlerts) {
            if (a.dedupKey == it.key()) { wasJustInserted = true; break; }
        }
        if (wasJustInserted) continue;
    ...
    for (const QString& k : resolvedKeys) {
        ...
        qInfo().noquote()
            << QString("[EDR] resolved %1 %2: %3 (key=%4, ticks=%5)")
                  .arg(EDR::severityToText(resolvedAlert.severity).toUpper(),
                       resolvedAlert.category, resolvedAlert.title,
    ...
    if (wasOurTick) {
        ...
        qInfo().noquote()
            << QString("[EDR] tick complete new=%1 persist=%2 resolved=%3 "
                         "active=%4 duration=%5ms")
                  .arg(trulyNew).arg(updated).arg(resolved)
                  .arg(m_active.size()).arg(ms);
