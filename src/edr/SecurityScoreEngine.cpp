...
QString scoreLabelToText(ScoreLabel l) {
  switch (l) {
    case ScoreLabel::Secure:
      return "Secure";
    case ScoreLabel::Moderate:
      return "Moderate";
    case ScoreLabel::HighRisk:
      return "High Risk";
  }
  return "Unknown";
}
...
ScoreReport scoreActiveAlerts(const QHash<QString, Alert>& activeAlerts) {
  ...
  for (auto it = activeAlerts.constBegin(); it != activeAlerts.constEnd(); ++it) {
    ...
    const Severity effective = isCrossView(a) ? Severity::Low : a.severity;
    ...
    switch (effective) {
      ...
      case Severity::Info:
        ++lowN;
        break;  // bucket Info as Low
    }

    if (a.ticksSeen >= kPersistenceMinTicks)
      ++persistentN;
  }
  ...
  push(
      dCrit,
      critN == 1 ? QStringLiteral("1 critical alert") : QString("%1 critical alerts").arg(critN));
  push(
      dHigh,
      highN == 1 ? QStringLiteral("1 high-severity alert")
                 : QString("%1 high-severity alerts").arg(highN));
  if (dMed > 0) {
    QString s =
        (medN == 1 ? QStringLiteral("1 medium-severity alert")
  ...
  if (dLow > 0) {
    QString s =
        (lowN == 1 ? QStringLiteral("1 low / info alert")
  ...
  push(
      dPersist,
      persistentN == 1 ? QString("1 alert persisting across %1+ ticks").arg(kPersistenceMinTicks)
