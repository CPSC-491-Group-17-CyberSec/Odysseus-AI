...
const char* severityHex(Severity s)
{
    ...
    switch (s) {
        case Severity::Critical: return "#EF4444";   // red
        case Severity::High:     return "#F59E0B";   // amber
        case Severity::Medium:   return "#F59E0B";   // amber
        case Severity::Low:      return "#10B981";   // green
        case Severity::Info:     return "#3B82F6";   // accent blue
    }
    return "#3B82F6";
}
...
QString keyForProcess(const SuspiciousProcess& sp) {
    const QString anchor = sp.info.exePath.isEmpty()
                              ? sp.info.name : sp.info.exePath;
    return QString("process|%1").arg(anchor);
}

void diffSuspiciousProcesses(const SystemSnapshot& prev,
                              ...
                              QHash<QString, EDR::Alert>& currentKeys)
{
    ...
    for (const SuspiciousProcess& sp : curr.suspicious) {
        ...
        a.description = sp.reasons.isEmpty()
            ? QStringLiteral("A suspicious process is running.")
            : sp.reasons.first();
        ...
        detail += QString("PID: %1\nPPID: %2\nUser: %3\nExe: %4\n")
                      .arg(sp.info.pid).arg(sp.info.ppid)
...
void diffPersistence(const SystemSnapshot& prev,
                      const SystemSnapshot& curr,
                      QVector<EDR::Alert>&  out,
                      QHash<QString, EDR::Alert>& currentKeys)
{
    ...
    for (const PersistenceItem& p : curr.persistence) {
        ...
        a.description = QString("A %1 entry exists. Persistence items run "
                                  "automatically on login or boot — verify "
                                  "it's expected.").arg(p.type);
        ...
        if (!p.filePath.isEmpty())  detail += "File: " + p.filePath + "\n";
        if (!p.program.isEmpty())   detail += "Program: " + p.program + "\n";
        ...
        if (!p.notes.isEmpty()) {
            detail += "\nNotes:\n";
            for (const QString& n : p.notes) detail += " - " + n + "\n";
        }
        a.rawDetail = detail.trimmed();
...
QString keyForCrossView(const CrossViewFinding& f) {
    ...
    const QString anchor = f.name.isEmpty()
                              ? QString::number(f.pid)
                              : f.name;
    return QString("crossview|%1|%2").arg(anchor, f.visibleIn);
}

void diffCrossView(const SystemSnapshot& prev,
                    const SystemSnapshot& curr,
                    QVector<EDR::Alert>&  out,
                    QHash<QString, EDR::Alert>& currentKeys)
{
    ...
    for (const CrossViewFinding& f : curr.rootkit.crossView) {
        ...
        a.rawDetail = QString("PID: %1\nName: %2\nVisible in: %3\nReason: %4")
                          .arg(f.pid).arg(f.name, f.visibleIn, f.reason);
...
void diffIntegrity(const SystemSnapshot& prev,
                    const SystemSnapshot& curr,
                    QVector<EDR::Alert>&  out,
                    QHash<QString, EDR::Alert>& currentKeys)
{
    ...
    for (const IntegrityFinding& f : curr.rootkit.integrity) {
        if (f.status.toLower() != "mismatch") continue;
        ...
        a.description = "A critical system binary's SHA-256 differs from the "
        ...
        QString detail;
        detail += "Path: " + f.path + "\n";
        if (!f.expectedHash.isEmpty()) detail += "Baseline SHA-256: "
                                                  + f.expectedHash + "\n";
        if (!f.currentHash.isEmpty())  detail += "Current SHA-256:  "
                                                  + f.currentHash + "\n";
        if (f.currentSize > 0)         detail += QString("Size: %1 bytes\n")
                                                    .arg(f.currentSize);
        if (!f.note.isEmpty())          detail += "\n" + f.note;
        a.rawDetail = detail.trimmed();
...
void diffKernelExtensions(const SystemSnapshot& prev,
                           const SystemSnapshot& curr,
                           QVector<EDR::Alert>&  out,
                           QHash<QString, EDR::Alert>& currentKeys)
{
    ...
    for (const KernelExtension& k : curr.rootkit.extensions) {
        ...
        if (k.isApple && k.severity.toLower() == "low") continue;
        ...
        a.title     = QString("%1: %2")
                          .arg(k.source == "system_extension"
                                  ? "System extension"
                                  : "Kernel module")
                          .arg(k.bundleId);
        a.description = "A kernel-resident or system-extension module is "
        ...
        a.signingStatus = k.isApple ? 2 :
                          (k.signedBy.isEmpty() ? 0 : 1);
        a.heuristics  = k.notes;
        QString detail;
        detail += "Bundle ID: " + k.bundleId + "\n";
        if (!k.version.isEmpty())   detail += "Version: " + k.version + "\n";
        if (!k.teamId.isEmpty())    detail += "Team ID: " + k.teamId + "\n";
        if (!k.signedBy.isEmpty())  detail += "Signed by: " + k.signedBy + "\n";
        if (!k.state.isEmpty())     detail += "State: " + k.state + "\n";
        if (!k.notes.isEmpty()) {
            detail += "\nNotes:\n";
            for (const QString& n : k.notes) detail += " - " + n + "\n";
        }
        a.rawDetail = detail.trimmed();
