...
ScanContext FileScannerWorker::detectContext(const QString& rootPath) {
  ...
  ctx.isRemovable =
      (ctx.fsType == "vfat" || ctx.fsType == "fat" || ctx.fsType == "fat32" ||
       ctx.fsType == "exfat");

  if (!ctx.isWindowsFs && !ctx.isLinuxFs && !ctx.isMacFs && !ctx.isNetworkFs) {
    if (ctx.runningOnWindows)
      ctx.isWindowsFs = true;
    else if (ctx.runningOnMac)
      ctx.isMacFs = true;
    else
      ctx.isLinuxFs = true;
  }
...
void FileScannerWorker::buildFilterLists() {
  ...
  m_noHashExtensions = {
      ...
      "pk3",
      "pk4",   // Quake III / Quake 4 pak
      "pak",   // Quake I/II pak (also Chromium above)
      "res",   // Half-Life resource file
      "xtx",
      "dds",   // DirectDraw Surface / Nintendo textures
      "pvr",   // PowerVR texture
      "ktx",   // Khronos texture
