...
void ensureYaraInitialized() {
  ...
  if (!cfg.yaraEnabled) {
    qInfo().noquote() << "[YARA] disabled in configuration "
  ...
  if (rulesDir.isEmpty()) {
    qWarning().noquote() << "[YARA] no yara_rules directory found near" << appDir
...
ReputationDB* getReputationDB() {
  ...
  if (!db->open(dataDir, seed)) {
    delete db;
    qWarning().noquote() << "[Reputation] FAILED to open DB at" << dataDir
  ...
  qInfo().noquote() << QString("[Reputation] DB ready at %1 — %2 row(s) loaded%3")
                           .arg(db->path())
                           .arg(rows)
                           .arg(
                               seed.isEmpty() ? QStringLiteral(" (no seed file found)")
                                              : QString(" (seed: %1)").arg(seed));
  if (rows == 0 && seed.isEmpty()) {
    qWarning().noquote() << "[Reputation] DB is empty and no seed file was found. "
...
// ============================================================================
ReputationDB* odysseus_getReputationDB() {
  return getReputationDB();
}
...
bool checkByYara(
  ...
  outReason = QString("YARA rule(s) fired: %1\nFamily: %2\nSeverity: %3")
                  .arg(
                      ruleNames.join(", "),
                      primaryFamily.isEmpty() ? QStringLiteral("(unspecified)") : primaryFamily,
