...
ScannerConfig ScannerConfig::fromJson(const QJsonObject& o)
{
    ...
    c.processScanEnabled =
        jsonBool(o, "processScanEnabled", defaults.processScanEnabled);
    c.persistenceScanEnabled =
        jsonBool(o, "persistenceScanEnabled", defaults.persistenceScanEnabled);
    c.suspiciousProcessHeuristicsEnabled =
        jsonBool(o, "suspiciousProcessHeuristicsEnabled",
                 defaults.suspiciousProcessHeuristicsEnabled);
    // Phase 3
...
{
    if (!g_cachedPath.isEmpty()) return g_cachedPath;
...
bool writeJson(const QString& path, const QJsonObject& obj)
{
    ...
    if (!f.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
        qWarning().noquote() << "[Config] cannot write" << path
                             << "—" << f.errorString();
        return false;
    }
...
ScannerConfig loadFromDisk()
{
    ...
    if (!f.open(QIODevice::ReadOnly)) {
        qWarning().noquote() << "[Config] cannot read" << path
                             << "— using defaults (" << f.errorString() << ")";
    ...
    if (err.error != QJsonParseError::NoError || !doc.isObject()) {
        qWarning().noquote() << "[Config] parse error in" << path
                             << "—" << err.errorString()
                             << "— using defaults";
        return ScannerConfig{};
    }
...
void ensureLoaded()
{
    ...
    qInfo().noquote().nospace()
        ...
        << " heur="     << (g_config.suspiciousProcessHeuristicsEnabled ? "on" : "off")
