...
ThreatCard::ThreatCard(QWidget* parent)
  ...
  fileNameLabel->setStyleSheet("font-size: 13px; color: #8888AA; padding-bottom: 4px;");
...
void ThreatCard::setAnomalyScore(float score, float threshold) {
  ...
  scoreTextLabel->setText(
      QString("%1 / 1.000  (threshold: %2)").arg(score, 0, 'f', 3).arg(threshold, 0, 'f', 3));

  // Color the bar based on severity
  QString color;
  if (score >= 0.90f)
    color = "#FF4757";  // Critical red
  else if (score >= 0.75f)
    color = "#FF6348";  // High orange-red
  else if (score >= 0.60f)
    color = "#FFA502";  // Medium orange
  else
    color = "#FECA57";  // Low yellow
...
void ThreatCard::updateSeverityStyle(const QString& level) {
  ...
  severityLabel->setStyleSheet(QString("font-size: 12px; font-weight: bold; color: #FFFFFF;"
