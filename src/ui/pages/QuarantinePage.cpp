...
QString relTimeFromEpoch(std::int64_t epochSeconds)
{
    ...
    else if (secs < 60*60)     return QString("%1m ago").arg(secs / 60);
    else if (secs < 60*60*24)  return QString("%1h ago").arg(secs / 3600);
...
void QuarantinePage::buildUi()
{
    ...
    connect(m_refreshBtn, &QPushButton::clicked,
            this,         &QuarantinePage::onRefreshClicked);
    ...
    leftTitle->setStyleSheet(QString(
    ...
    m_emptyHint->setStyleSheet(QString(
        "QLabel { color: %1; %2 padding: 24px; background: transparent; }")
    ...
    m_detail->setStyleSheet(QString(
        "QLabel { color: %1; %2 background: transparent;"
        " padding: 6px 0; }")
            .arg(Theme::Color::textPrimary)
            .arg(Theme::Type::qss(Theme::Type::Body)));
    ...
    m_restoreBtn->setStyleSheet(QString(
    ...
    connect(m_restoreBtn, &QPushButton::clicked,
            this,         &QuarantinePage::onRestoreClicked);
...
void QuarantinePage::refresh()
{
    ...
    for (const R::QuarantineEntry& e : entries) {
        ...
        const QString name = origInfo.fileName().isEmpty()
                                 ? QStringLiteral("(unnamed)")
                                 : origInfo.fileName();
        const QString rel = relTimeFromEpoch(e.timestampEpoch);

        auto* item = new QListWidgetItem(QString("%1   ·   %2").arg(name, rel),
                                          m_list);
    ...
    if (m_emptyHint) m_emptyHint->setVisible(empty);
    if (empty) {
        m_detail->setText("Select a file on the left to view details.");
        m_restoreBtn->setEnabled(false);
...
void QuarantinePage::onSelectionChanged()
{
    ...
    const QString sha256    = e.sha256.empty()
                                  ? QStringLiteral("(not recorded)")
                                  : QString::fromStdString(e.sha256);
    const QString reason    = e.reason.empty()
                                  ? QStringLiteral("(none)")
                                  : QString::fromStdString(e.reason).toHtmlEscaped();
    const QString when      = (e.timestampEpoch > 0)
        ? QDateTime::fromSecsSinceEpoch(e.timestampEpoch)
    ...
    const QString html = QString(
        ...
        "</table>")
        .arg(QFileInfo(QString::fromStdString(e.originalPath))
                 .fileName().toHtmlEscaped(),
             origPath, quarPath, sha256, when, reason);
...
void QuarantinePage::onRestoreClicked()
{
    ...
    box.setInformativeText(QString(
        ...
        "what to do (the safe default is to restore-with-new-name).")
            .arg(QFileInfo(QString::fromStdString(opt->originalPath))
                     .fileName().toHtmlEscaped(),
    ...
    if (res.needsUserChoice) {
        ...
        QPushButton* btnNew    = conflict.addButton(
            "Restore with new name",  QMessageBox::AcceptRole);
        QPushButton* btnOver   = conflict.addButton(
            "Overwrite existing",     QMessageBox::DestructiveRole);
        QPushButton* btnCancel = conflict.addButton(
        ...
        req.restorePolicy = (clicked == btnOver)
            ? R::RestoreConflictPolicy::Overwrite
    ...
    if (res.success) {
        QMessageBox::information(this, "Restore",
            QString("File restored to:\n%1")
                .arg(QString::fromStdString(res.newPath)));
        setStatus(QString("Restored to %1")
                      .arg(QString::fromStdString(res.newPath)),
                  false);
        refresh();
    } else {
        ...
        setStatus(QString("Restore failed: %1").arg(QString::fromStdString(err)),
                  true);
    }
}
