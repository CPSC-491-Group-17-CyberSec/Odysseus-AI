...
void ScanPage::buildUi()
{
    ...
    auto* subtitle = new QLabel("Configure and run AI-powered threat scans",
                                  content);
...
QFrame* ScanPage::buildTargetsCard()
{
    ...
    connect(m_dropArea, &DropArea::filesDropped,
            this, &ScanPage::onTargetsDropped);
    ...
    connect(m_btnSelectFiles, &QPushButton::clicked,
            this, &ScanPage::onSelectFiles);
    ...
    .arg(Theme::Color::bgSecondary, Theme::Color::textPrimary,
         Theme::Color::borderSubtle, Theme::Color::bgCardHover));
...
QFrame* ScanPage::buildEngineCard()
{
    ...
    modelLab->setStyleSheet(QString(
        "QLabel { color: %1; %2 background: transparent; }")
            .arg(Theme::Color::textPrimary)
            .arg(Theme::Type::qss(Theme::Type::H3,
                                    Theme::Type::WeightSemi)));
    ...
    m_modelStatus->setStyleSheet(QString(
    ...
    m_aiModel->addItems({ "ONNX Anomaly Model v2",
                            "EMBER LightGBM (PE specialist)",
                            "Hash-only mode" });
    v->addWidget(m_aiModel);

    m_modelHelp = new QLabel("Optimized for anomaly detection", card);
    m_modelHelp->setStyleSheet(QString(
        "QLabel { color: %1; %2 background: transparent; }")
            .arg(Theme::Color::textMuted)
            .arg(Theme::Type::qss(Theme::Type::Caption)));
    ...
    detectLab->setStyleSheet(QString(
        "QLabel { color: %1; %2 background: transparent; }")
            .arg(Theme::Color::textPrimary)
            .arg(Theme::Type::qss(Theme::Type::H3,
                                    Theme::Type::WeightSemi)));
    ...
    auto buildOpt = [&](const QString& label, const QString& sub,
                        bool defaultOn) -> QCheckBox*
    {
    ...
    m_optRootkit   = buildOpt("Rootkit Detection",
                                "Scan for hidden rootkits and kernel-level threats",
                                true);
    m_optMemory    = buildOpt("Memory Analysis",
                                "Analyze running processes and memory patterns",
                                true);
    m_optHeuristic = buildOpt("Heuristic Analysis",
                                "Use AI heuristics to detect unknown threats",
                                true);
    ...
    depthLab->setStyleSheet(QString(
        "QLabel { color: %1; %2 background: transparent; }")
            .arg(Theme::Color::textPrimary)
            .arg(Theme::Type::qss(Theme::Type::H3,
                                    Theme::Type::WeightSemi)));
    ...
    m_depthHelp->setStyleSheet(QString(
        "QLabel { color: %1; %2 background: transparent; }")
            .arg(Theme::Color::textMuted)
            .arg(Theme::Type::qss(Theme::Type::Caption)));
    ...
    m_startSubtitle->setStyleSheet(QString(
        "QLabel { color: %1; %2 background: transparent; }")
            .arg(Theme::Color::textMuted)
            .arg(Theme::Type::qss(Theme::Type::Caption)));
...
QFrame* ScanPage::buildSettingsCard()
{
    ...
    connect(m_btnAdvanced, &QPushButton::clicked,
            this, &ScanPage::onAdvancedSettingsClicked);
...
QFrame* ScanPage::buildRecentScansCard()
{
    ...
    m_btnViewAll->setStyleSheet(QString(
     ...
     .arg(Theme::Color::accentBlueHover));
    connect(m_btnViewAll, &QPushButton::clicked,
            this, &ScanPage::viewAllRecentRequested);
    ...
    connect(m_btnExport, &QPushButton::clicked,
            this, &ScanPage::exportLogsRequested);
...
void ScanPage::onSelectFiles()
{
    ...
    const QStringList paths = QFileDialog::getOpenFileNames(
        this, "Select files to scan", home);
    if (!paths.isEmpty()) onTargetsDropped(paths);
}
...
void ScanPage::onSelectDrive()
{
    ...
    const QString dir = QFileDialog::getExistingDirectory(
        this, "Select drive or folder to scan",
        start, QFileDialog::ShowDirsOnly);
    if (!dir.isEmpty()) onTargetsDropped({ dir });
}

void ScanPage::onTargetsDropped(const QStringList& paths)
{
    for (const QString& p : paths) {
        if (!m_targets.contains(p)) m_targets.append(p);
    }
    rebuildTargetList();
}
...
{
    if (!m_btnStart || !m_btnStart->isEnabled()) return;
    const int depth = m_scanDepth ? m_scanDepth->currentData().toInt()
                                    : Standard;
...
{
    if (!m_targetList) return;
    m_targetList->clear();

    if (m_targets.isEmpty()) {
        auto* item = new QListWidgetItem("No targets selected. Drag files in "
        ...
        for (const QString& path : m_targets) {
            ...
            auto* iconLab = new QLabel(fi.isDir()
                                          ? QString::fromUtf8("\xE2\x96\xA4") // ▤
                                          : QString::fromUtf8("\xE2\x96\xA1"),// □
                                         row);
            iconLab->setStyleSheet(QString(
                "QLabel { color: %1; %2 background: transparent; }")
                    .arg(Theme::Color::accentBlue)
                    .arg(Theme::Type::qss(Theme::Type::Body)));
            h->addWidget(iconLab);
            ...
            auto* sizeLab = new QLabel(
                fi.isFile() ? prettyBytes(sz) : "—", row);
            sizeLab->setStyleSheet(QString(
                "QLabel { color: %1; %2 background: transparent; }")
                    .arg(Theme::Color::textSecondary)
                    .arg(Theme::Type::qss(Theme::Type::Caption)));
...
void ScanPage::rebuildRecentScans()
{
    ...
    for (int i = 0; i < maxRows; ++i) {
        ...
        filesLab->setStyleSheet(QString(
            "QLabel { color: %1; %2 background: transparent; }")
                .arg(Theme::Color::textSecondary)
                .arg(Theme::Type::qss(Theme::Type::Caption)));
        ...
        auto* threatLab = new QLabel(
            clean ? "No threats" : QString("%1 threat%2 found")
                                       .arg(totalThreats)
                                       .arg(totalThreats == 1 ? "" : "s"),
            row);
        threatLab->setAlignment(Qt::AlignRight);
        threatLab->setStyleSheet(QString(
            "QLabel { color: %1; %2 background: transparent; }")
                .arg(clean ? Theme::Color::severitySafe
...
// ============================================================================
void ScanPage::setStats(const QDateTime& lastScan,
                          int            filesScanned,
                          int            threatsFound,
                          bool           protectedNow,
                          bool           scanning)
{
    ...
    if (m_kpiThreatsFound) {
        ...
        m_kpiThreatsFound->setSubtitle(threatsFound > 0
                                         ? "Requires attention"
                                         : "No threats detected");
    }
...
void ScanPage::setScanning(bool scanning)
{
    ...
    if (m_startSubtitle)
        m_startSubtitle->setText(scanning
                                   ? "Findings will appear under Results"
