...
void ResultsPage::buildUi() {
  ...
  m_exportBtn->setStyleSheet(
      QString("QPushButton {"
          ...
          .arg(Theme::Color::bgCard, Theme::Color::textPrimary, Theme::Color::borderSubtle)
          .arg(Theme::Type::qss(Theme::Type::Body, Theme::Type::WeightSemi))
          .arg(Theme::Color::accentBlueSoft));
  connect(m_exportBtn, &QPushButton::clicked, this, &ResultsPage::exportRequested);
  ...
  connect(m_searchInput, &QLineEdit::textChanged, this, &ResultsPage::onSearchChanged);
  ...
  m_severityFilter->addItems({"All Severities", "Critical", "Suspicious", "Needs Review"});
  styleCombo(m_severityFilter);
  connect(m_severityFilter, &QComboBox::currentTextChanged, this, &ResultsPage::onFilterChanged);
  filterRow->addWidget(m_severityFilter, 0);

  m_sourceFilter = new QComboBox(content);
  m_sourceFilter->addItems({"All Sources", "AI Model", "NVD", "YARA", "Hash DB"});
  styleCombo(m_sourceFilter);
  connect(m_sourceFilter, &QComboBox::currentTextChanged, this, &ResultsPage::onFilterChanged);
  ...
  headerBar->setStyleSheet(QString("QFrame { background: transparent;"
  ...
  m_emptyState = new QLabel("No findings yet. Run a scan from the Dashboard.", m_listCard);
  m_emptyState->setAlignment(Qt::AlignCenter);
  m_emptyState->setStyleSheet(
      QString("QLabel { color: %1; %2 background: transparent; padding: 60px 40px; }")
          .arg(Theme::Color::textMuted)
          .arg(Theme::Type::qss(Theme::Type::Body)));
  listLayout->addWidget(m_emptyState);

  m_resultsCount = new QLabel("0 results", m_listCard);
  m_resultsCount->setStyleSheet(QString("QLabel { color: %1; %2 background: transparent;"
  ...
  m_detailPanel->setStyleSheet(QString("QFrame#OdyDetailPanel {"
                                       "  background-color: %1;"
                                       "  border: 1px solid %2;"
                                       "  border-radius: 12px;"
                                       "}")
  ...
  // ── Detail empty state ─────────────────────────────────────────────
  m_detailEmpty = new QLabel("Select a threat to see details.", m_detailPanel);
  m_detailEmpty->setAlignment(Qt::AlignCenter);
  m_detailEmpty->setStyleSheet(
      QString("QLabel { color: %1; %2 background: transparent; padding: 60px 20px; }")
          .arg(Theme::Color::textMuted)
          .arg(Theme::Type::qss(Theme::Type::Body)));
  ...
  m_detailSeverity->setStyleSheet(
      QString("QLabel { color: white; background-color: %1;"
  ...
  m_detailConfText->setStyleSheet(QString("QLabel { color: %1; %2 background: transparent; }")
                                      .arg(Theme::Color::textSecondary)
  ...
  m_detailConfBar->setStyleSheet(
      QString("QProgressBar { background-color: %1; border-radius: 2px; }"
  ...
  auto buildKV = [this, fi](const QString& key, QLabel*& valOut, bool monospace = false) {
    ...
    const QString vqss = monospace ? QString(
                                         "QLabel { color: %1; %2 background: transparent;"
  ...
  m_secWhyFlagged =
      new DetailSection("Why was this flagged?", Theme::Color::severityCritical, m_detailPanel);
  dp->addWidget(m_secWhyFlagged);

  m_secAiSummary = new DetailSection("AI Summary", Theme::Color::accentBlue, m_detailPanel);
  dp->addWidget(m_secAiSummary);

  m_secIndicators = new DetailSection("Indicators", Theme::Color::severityMedium, m_detailPanel);
  dp->addWidget(m_secIndicators);

  m_secActions =
      new DetailSection("Recommended Actions", Theme::Color::severitySafe, m_detailPanel);
  ...
  m_btnQuarantine->setStyleSheet(
      QString("QPushButton { background-color: %1; color: white; border: none;"
          ...
          .arg(Theme::Color::textMuted));
  connect(m_btnQuarantine, &QPushButton::clicked, this, &ResultsPage::onQuarantineClicked);
  ...
  auto styleSecondaryBtn = [](QPushButton* b) {
    ...
    b->setStyleSheet(QString("QPushButton { background: transparent; color: %1;"
  ...
  m_btnDelete->setToolTip("Disabled by design — use Quarantine (reversible) instead.");
  ...
  m_btnIgnore->setToolTip("Add this file to the allowlist (suppress future findings)");
  connect(m_btnIgnore, &QPushButton::clicked, this, &ResultsPage::onIgnoreClicked);
  ...
  auto setSiblingsVisible = [&](bool v) {
    ...
    if (m_btnQuarantine)
      m_btnQuarantine->setVisible(v);
    if (m_btnDelete)
      m_btnDelete->setVisible(v);
    if (m_btnIgnore)
      m_btnIgnore->setVisible(v);
    // File-info kv labels share parent — toggle parent
    if (m_detailSource)
      m_detailSource->parentWidget()->setVisible(v);
  };
...
bool ResultsPage::rowMatchesFilter(const SuspiciousFile& sf) const {
  // Search
  const QString q = m_searchInput ? m_searchInput->text().trimmed().toLower() : QString();
  if (!q.isEmpty()) {
    const bool hit = sf.fileName.toLower().contains(q) || sf.filePath.toLower().contains(q) ||
                     sf.cveId.toLower().contains(q) || sf.category.toLower().contains(q) ||
  ...
  // Severity filter
  const QString sevSel = m_severityFilter ? m_severityFilter->currentText() : "All Severities";
  ...
  // Source filter
  const QString srcSel = m_sourceFilter ? m_sourceFilter->currentText() : "All Sources";
  if (srcSel != "All Sources") {
    if (sourceForFinding(sf) != srcSel)
      return false;
  }
  return true;
}
...
void ResultsPage::rebuildVisibleRows() {
  ...
  for (int i = 0; i < m_findings.size(); ++i) {
    ...
    row->setConfidence(sf.anomalyScore > 0 ? sf.anomalyScore : sf.confidencePct / 100.0f);
    row->setSource(sourceForFinding(sf));
    row->setDetected(sf.lastModified.isValid() ? sf.lastModified.toString("MMM d, yyyy") : "—");
    row->setStatus("Detected");
    row->setSelected(i == m_selectedIndex);
    connect(row, &ThreatRow::clicked, this, &ResultsPage::onRowClicked);
  ...
  if (m_resultsCount)
    m_resultsCount->setText(QString("Showing %1 of %2 results").arg(kept).arg(m_findings.size()));
}
...
void ResultsPage::recomputeStats() {
  ...
  for (const SuspiciousFile& sf : m_findings) {
    ...
    if (sev == "critical")
      ++critical;
    else if (sev == "suspicious")
      ++suspicious;
    else if (sev == "needs-review")
      ++needsReview;
...
void ResultsPage::populateDetail(const SuspiciousFile& sf) {
  ...
  if (m_detailSource)
    setVis(m_detailSource->parentWidget(), true);
  ...
  if (m_btnQuarantine)
    m_btnQuarantine->setEnabled(havePath);
  if (m_btnIgnore)
    m_btnIgnore->setEnabled(havePath);
  ...
  // Severity badge
  const QString sev = severityFromFinding(sf);
  const QString sevHex = (sev == "critical") ? Theme::Color::severityCritical
                         : (sev == "clean")  ? Theme::Color::severitySafe
                                             : Theme::Color::severityMedium;
  QString sevText = (sev == "critical")       ? "Critical"
  ...
  // Confidence
  const float pct = sf.anomalyScore > 0 ? sf.anomalyScore : sf.confidencePct / 100.0f;
  m_detailConfText->setText(QString("Confidence: %1").arg(pct, 0, 'f', 3));
  ...
  if (sf.aiSummary.isEmpty()) {
    ...
    QStringList sentences =
        sf.aiSummary.split(QRegularExpression("(?<=[.!?])\\s+"), Qt::SkipEmptyParts);
...
void ResultsPage::onQuarantineClicked() {
  ...
  if (sf.filePath.isEmpty()) {
    QMessageBox::warning(this, "Quarantine", "No file path available for this finding.");
    return;
  }
  ...
  box.setInformativeText(QString("<b>%1</b><br><br>"
                                 "<i>%2</i><br><br>"
                                 ...
                                 "action log.")
                             .arg(sf.fileName.toHtmlEscaped(), sf.filePath.toHtmlEscaped()));
  ...
  req.reason = sf.reason.isEmpty() ? std::string(
                                         "User-initiated quarantine "
                                         "from Results page")
  ...
  if (res.success) {
    ...
    QMessageBox::critical(
        ...
        QString("Could not quarantine file.\n\n%1")
            .arg(
                QString::fromStdString(res.errorMessage.empty() ? res.message : res.errorMessage)));
  }
}
...
void ResultsPage::onIgnoreClicked() {
  ...
  const SuspiciousFile& sf = m_findings[m_selectedIndex];
  if (sf.filePath.isEmpty()) {
    QMessageBox::warning(this, "Ignore", "No file path available for this finding.");
    return;
  }

  // Confirmation — adding to the allowlist suppresses future findings
  // for this file. We prefer SHA-256 entries when available.
  const QString idLine = sf.sha256.isEmpty() ? QString("Path: %1").arg(sf.filePath.toHtmlEscaped())
