    ...
    : QWidget(parent)
{
    ...
    m_welcomeTitle = new QLabel(
        QString("Welcome back, User \xF0\x9F\x91\x8B"), content);
    m_welcomeTitle->setStyleSheet(QString(
        "color: %1; font-size: 24px; font-weight: 700;")
            .arg(Theme::Color::textPrimary));
    welcomeBox->addWidget(m_welcomeTitle);

    m_welcomeSub = new QLabel(
        "Here's what's happening with your system today.", content);
    m_welcomeSub->setStyleSheet(QString(
    ...
    overview->setStyleSheet(QString(
        "QFrame#OdyOverview {"
        "  background-color: %1;"
        "  border: 1px solid %2;"
        "  border-radius: %3px;"
    ...
    weekLabel->setStyleSheet(QString(
        "color: %1; font-size: 11px; padding: 4px 10px;"
    ...
    auto buildLegendRow = [overview](const QString& dotColor,
                                       const QString& label,
                                       QLabel*& valueOut) {
        ...
        lab->setStyleSheet(QString(
            "color: %1; font-size: 12px;").arg(Theme::Color::textSecondary));
        h->addWidget(lab, 1);
        valueOut = new QLabel("0", overview);
        valueOut->setStyleSheet(QString(
    ...
    // Forward signals
    connect(m_scanSelector, &ScanTypeSelector::scanRequested,
            this,           &DashboardPage::scanRequested);
    connect(m_activity,     &ActivityList::viewAllClicked,
            this,           &DashboardPage::viewAllActivityClicked);

    // Stash overview pointer so refresh() can find legend labels.
    setProperty("__overview", QVariant::fromValue<QObject*>(overview));
}

void DashboardPage::refresh(const QVector<SuspiciousFile>& findings,
                             ...
                             const SystemSnapshot*           sysSnapshot)
{
    ...
    for (const SuspiciousFile& sf : findings) {
        ...
        if (c == "CRITICAL")        ++criticalN;
        else if (c == "SUSPICIOUS") ++suspN;
        else                         ++lowN;
    }
    ...
    m_cardCritical->setSubtitle(criticalN > 0
                                  ? "Requires immediate attention"
                                  : "All clear");
    m_cardSuspicious->setValue(QString::number(suspN));
    m_cardSuspicious->setSubtitle(suspN > 0
                                    ? "Review recommended"
                                    : "No suspicious files");
    ...
    auto* overview = qobject_cast<QFrame*>(
        property("__overview").value<QObject*>());
    if (overview) {
        auto setLegend = [overview](const char* prop, int v, int total) {
            auto* lab = qobject_cast<QLabel*>(
                overview->property(prop).value<QObject*>());
            if (!lab) return;
            const double pct = total > 0 ? 100.0 * v / total : 0.0;
            lab->setText(QString("%1 (%2%)")
                          .arg(v)
                          .arg(QString::number(pct, 'f', total > 0 ? 0 : 0)));
        };
    ...
    for (int i = qMin(3, findings.size()) - 1; i >= 0; --i) {
        ...
        if      (c == "CRITICAL")   severityLabel = "Critical threat detected";
        else if (c == "SUSPICIOUS") severityLabel = "Suspicious file detected";
        else if (c == "ANOMALOUS")  severityLabel = "Anomalous file detected";
        else                         severityLabel = "Threat detected";

        e.title    = QString("%1: %2").arg(severityLabel, sf.fileName);
        e.subtitle = sf.filePath;
        e.when     = sf.lastModified.isValid() ? sf.lastModified
    ...
    for (int i = 0; i < qMin(3, history.size()); ++i) {
        ...
        ActivityList::Entry e;
        const bool clean = (r.criticalCount == 0
                              && r.suspiciousOnly == 0
                              && r.reviewCount == 0);
        e.tone     = clean ? ActivityList::Success : ActivityList::Info;
        e.title    = clean
                       ? QString("Scan completed — system clean")
    ...
    if (sysSnapshot) {
        for (const SuspiciousProcess& sp : sysSnapshot->suspicious) {
            ...
            if      (sev == "high")   ++procHigh;
            else if (sev == "medium") ++procMed;
        }
