...
void AlertsPage::buildUi()
{
    ...
    m_subtitle = new QLabel(
        "EDR-Lite alerts from continuous system monitoring", content);
    ...
    m_timeline->setStyleSheet(QString(
        "QFrame#OdyAlertTimeline {"
    ...
    m_timelineDot->setStyleSheet(QString(
    ...
    m_timelineText->setStyleSheet(QString(
        "QLabel { color: %1; %2 background: transparent; }")
            .arg(Theme::Color::textSecondary)
            .arg(Theme::Type::qss(Theme::Type::Body)));
    ...
    connect(m_timelineTick, &QTimer::timeout,
            this, &AlertsPage::onTimelineRefresh);
    ...
    connect(m_filterBar, &FilterBar::filtersChanged,
            this, &AlertsPage::onFiltersChanged);
    ...
    m_emptyState = new QLabel(
        ...
        "start watching for system changes.", m_listHost);
    ...
    m_loadingState = new QLabel(
        "Checking system for changes…", m_listHost);
    m_loadingState->setAlignment(Qt::AlignCenter);
    m_loadingState->setWordWrap(true);
    m_loadingState->setStyleSheet(QString(
        "QLabel { color: %1; %2 background: transparent;"
        " padding: 60px 40px; }")
            .arg(Theme::Color::textMuted)
    ...
    connect(m_detail, &AlertDetailPanel::openLocationRequested,
            this, &AlertsPage::onOpenLocationRequested);
...
void AlertsPage::setLastTick(const QDateTime& when, int alertsThisTick)
{
    ...
    if (m_loadingState) m_loadingState->setVisible(false);
}
...
bool AlertsPage::passesFilters(const EDR::Alert& a) const
{
    ...
    if (sevSel >= 0 && static_cast<int>(a.severity) != sevSel) return false;
    if (!cat.isEmpty() && a.category != cat) return false;
    if (!q.isEmpty()) {
        const QString needle = q.toLower();
        if (!a.title.toLower().contains(needle)
            && !a.sourcePath.toLower().contains(needle)
            && !a.category.toLower().contains(needle)
...
QVector<AlertsPage::Group> AlertsPage::groupAndFilter() const
{
    ...
    for (int i = 0; i < m_alerts.size(); ++i) {
        ...
        bool merged = false;
        for (Group& g : out) {
            if (g.representative.category   == a.category
                && g.representative.sourcePath == a.sourcePath) {

                // Time check: if the group's representative is within
                // kGroupWindowSecs of this alert, merge.
                const QDateTime ref = g.representative.timestamp.isValid()
                                           ? g.representative.timestamp
                                           : g.representative.lastSeen;
                const QDateTime cur = a.timestamp.isValid()
                                           ? a.timestamp
                                           : a.lastSeen;
                if (ref.isValid() && cur.isValid()
                    && qAbs(ref.secsTo(cur)) <= kGroupWindowSecs) {

                    g.occurrences++;
                    g.sourceIndices.append(i);
                    // Bump representative.lastSeen / firstSeen across the group
                    if (!g.representative.lastSeen.isValid()
                        || cur > g.representative.lastSeen) {
                        g.representative.lastSeen = cur;
                    }
                    if (!g.representative.firstSeen.isValid()
                        || cur < g.representative.firstSeen) {
...
void AlertsPage::rebuildList()
{
    ...
    for (int i = 0; i < m_currentGroups.size(); ++i) {
        ...
        row->setAlert(g.representative, i, g.occurrences,
                       /*isGroupHeader*/ g.occurrences > 1);
        row->setZebra(i % 2 == 1);
        connect(row, &AlertRow::clicked,
                this, &AlertsPage::onRowClicked);
        ...
        if (i == kVirtualizeThreshold) {
            ...
            auto* warn = new QLabel(
                QString("(showing first %1 of %2 alerts — refine filters to "
                        "narrow down)")
                    .arg(kVirtualizeThreshold).arg(m_currentGroups.size()),
                m_listHost);
...
void AlertsPage::refreshKpis()
{
    ...
    for (const EDR::Alert& a : m_alerts) {
        if (a.severity == EDR::Severity::Critical) ++critical;
        if (a.severity == EDR::Severity::High)     ++high;
        if (a.timestamp >= cutoff)                  ++recentHour;
    }
    if (m_kpiTotal)    m_kpiTotal->setValue(QString::number(total));
    if (m_kpiCritical) m_kpiCritical->setValue(QString::number(critical));
    if (m_kpiHigh)     m_kpiHigh->setValue(QString::number(high));
    if (m_kpiRecent)   m_kpiRecent->setValue(QString::number(recentHour));
}

void AlertsPage::refreshTimelineLabel()
{
    if (!m_timeline || !m_timelineDot || !m_timelineText) return;
...
{
    if (displayIndex < 0 || displayIndex >= m_rows.size()) return;
