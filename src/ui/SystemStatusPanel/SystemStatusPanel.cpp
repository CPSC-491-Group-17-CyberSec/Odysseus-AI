...
void SystemStatusPanel::buildUi()
{
    ...
    setStyleSheet(QString(
        "QFrame { background-color: %1; border-radius: 15px; color: %2; }")
    ...
    auto buildKpi = [this](const QString& label, QLabel*& valOut) {
        ...
        valOut->setStyleSheet(QString("color: %1; %2 background: transparent;")
                                  .arg(Theme::Color::textPrimary)
                                  .arg(Theme::Type::qss(Theme::Type::H1,
                                                          Theme::Type::WeightBold)));
        v->addWidget(lab);
        v->addWidget(valOut);
        return box;
    ...
    auto buildSection = [this, mainLayout]
        (const QString& title, const QString& color,
         QLabel*& headerOut, QListWidget*& listOut)
    {
        headerOut = new QLabel(title);
        headerOut->setStyleSheet(
            QString("QLabel { color: %1; %2 padding-top: 6px;"
    ...
    buildSection("Suspicious Processes (—)", "#C62828",
                 m_suspiciousHeader, m_suspiciousList);
    connect(m_suspiciousList, &QListWidget::itemClicked,
            this, &SystemStatusPanel::onProcessRowClicked);

    // ── Persistence section (Phase 2) ──────────────────────────────────
    buildSection("Persistence Items (—)", "#1565C0",
                 m_persistenceHeader, m_persistenceList);
    connect(m_persistenceList, &QListWidget::itemClicked,
            this, &SystemStatusPanel::onPersistenceRowClicked);

    // ── Cross-view findings (Phase 3) ──────────────────────────────────
    buildSection("Process Cross-View Findings (—)", "#6A1B9A",
                 m_crossViewHeader, m_crossViewList);
    connect(m_crossViewList, &QListWidget::itemClicked,
            this, &SystemStatusPanel::onCrossViewRowClicked);

    // ── Kernel/system extensions (Phase 3) ─────────────────────────────
    buildSection("Kernel / System Extensions (—)", "#00695C",
                 m_extensionsHeader, m_extensionsList);
    connect(m_extensionsList, &QListWidget::itemClicked,
            this, &SystemStatusPanel::onExtensionRowClicked);

    // ── Integrity findings (Phase 3) ───────────────────────────────────
    buildSection("Integrity Findings (—)", "#BF360C",
                 m_integrityHeader, m_integrityList);
    connect(m_integrityList, &QListWidget::itemClicked,
            this, &SystemStatusPanel::onIntegrityRowClicked);
    ...
    m_detailLabel->setStyleSheet(QString(
    ...
    connect(m_closeBtn, &QPushButton::clicked,
            this, &SystemStatusPanel::closeRequested);
}
...
void SystemStatusPanel::setSnapshot(const SystemSnapshot& snap)
{
    ...
    QString sub = QString("Last refresh: %1")
                      .arg(snap.capturedAt.toString("yyyy-MM-dd HH:mm:ss"));
    if (snap.restrictedCmdlines > 0) {
        sub += QString("  •  %1 process(es) had restricted metadata "
    ...
    if (!snap.suspicious.isEmpty())
        m_kpiSuspicious->setStyleSheet(
            "font-size: 22px; font-weight: bold; color: #C62828;");
    else
        m_kpiSuspicious->setStyleSheet(
    ...
    m_kpiPlatform->setStyleSheet(
        "font-size: 18px; font-weight: bold; color: #333;");

    // Section headers with counts
    m_suspiciousHeader->setText(
        QString("Suspicious Processes (%1)").arg(snap.suspicious.size()));
    m_persistenceHeader->setText(
        QString("Persistence Items (%1)").arg(snap.persistence.size()));
    ...
    if (snap.persistence.isEmpty()) {
        ...
        for (int i = 0; i < snap.persistence.size(); ++i) {
            ...
            const QString prog = pi.program.isEmpty()
                                     ? QStringLiteral("(no Program)")
                                     : pi.program;
            const QString summary = QString("[%1]  %2  →  %3")
                                        .arg(pi.type, pi.label, prog);
            auto* item = new QListWidgetItem(summary);
    ...
    // ── Cross-view list ────────────────────────────────────────────────
    m_crossViewHeader->setText(
        QString("Process Cross-View Findings (%1)").arg(rk.crossView.size()));
    ...
    if (!rk.ran) {
        ...
        auto* item = new QListWidgetItem(
            QString("No process-list disagreements (sysctl=%1, ps=%2).")
                .arg(rk.processSysctlCount).arg(rk.processPsCount));
        item->setForeground(QColor("#2E7D32"));
        item->setFlags(Qt::ItemIsEnabled);
        m_crossViewList->addItem(item);
    } else {
    ...
    // ── Extensions list ────────────────────────────────────────────────
    m_extensionsHeader->setText(
        QString("Kernel / System Extensions (%1)").arg(rk.extensions.size()));
    ...
    if (!rk.ran) {
        ...
        for (int i = 0; i < rk.extensions.size(); ++i) {
            ...
            const QString tag = k.isApple ? "Apple" : (k.teamId.isEmpty()
                                              ? "(no teamID)"
                                              : QString("teamID:%1").arg(k.teamId));
    ...
    // ── Integrity list ─────────────────────────────────────────────────
    m_integrityHeader->setText(
        QString("Integrity Findings (%1, %2 mismatch(es))")
            .arg(rk.integrity.size()).arg(rk.integrityMismatches));
    ...
    if (!rk.ran) {
        ...
        for (int i = 0; i < rk.integrity.size(); ++i) {
            ...
            const QString summary = QString("[%1]  %2  \xE2\x86\x92  %3")
                                        .arg(f.status.toUpper())
                                        .arg(f.path)
                                        .arg(f.note);
            auto* item = new QListWidgetItem(summary);
            item->setForeground(QColor(severityColor(f.severity)));
...
void SystemStatusPanel::onProcessRowClicked(QListWidgetItem* item)
{
    ...
    if (idx < 0 || idx >= m_snapshot.suspicious.size()) return;
    const SuspiciousProcess& sp = m_snapshot.suspicious[idx];

    QString html;
    html += QString("<b>PID %1 — %2</b>").arg(sp.info.pid).arg(sp.info.name);
    html += QString("<span style='background-color:%1; color:%2; padding:2px 8px;"
                    " border-radius:6px; margin-left:10px; font-weight:bold;'>%3</span><br>")
              .arg(severityBg(sp.severity), severityColor(sp.severity),
                   sp.severity.toUpper());

    html += QString("<span style='color:#666;'>User:</span> %1 &nbsp;"
                    "<span style='color:#666;'>PPID:</span> %2 &nbsp;"
                    "<span style='color:#666;'>Score:</span> %3<br>")
              .arg(sp.info.user).arg(sp.info.ppid).arg(sp.score);

    if (!sp.info.exePath.isEmpty()) {
        html += QString("<span style='color:#666;'>Exe:</span> "
                        "<code>%1</code>%2<br>")
                  .arg(sp.info.exePath)
    ...
    if (!sp.info.cmdLine.isEmpty()) {
        ...
        if (cl.length() > 200) cl = cl.left(200) + "…";
    ...
    if (sp.signingStatus >= 0) {
        ...
        switch (sp.signingStatus) {
            case 2: sgn = "<span style='color:#2E7D32;'>signed (trusted)</span>"; break;
            case 1: sgn = "<span style='color:#E65100;'>signed (untrusted)</span>"; break;
            case 0: sgn = "<span style='color:#C62828;'>UNSIGNED</span>"; break;
            default: sgn = "unknown"; break;
        }
...
void SystemStatusPanel::onPersistenceRowClicked(QListWidgetItem* item)
{
    if (!item) return;
    const int idx = item->data(Qt::UserRole).toInt();
    if (idx < 0 || idx >= m_snapshot.persistence.size()) return;
    ...
    html += QString("<span style='background-color:%1; color:%2; padding:2px 8px;"
                    " border-radius:6px; margin-left:10px; font-weight:bold;'>%3</span><br>")
              .arg(severityBg(pi.severity), severityColor(pi.severity),
                   pi.severity.toUpper());
    ...
    if (pi.runAtLoad)  html += "RunAtLoad: <b>YES</b><br>";
    if (pi.keepAlive)  html += "KeepAlive: <b>YES</b><br>";
    if (!pi.notes.isEmpty()) {
...
void SystemStatusPanel::onCrossViewRowClicked(QListWidgetItem* item)
{
    if (!item) return;
    const int idx = item->data(Qt::UserRole).toInt();
    if (idx < 0 || idx >= m_snapshot.rootkit.crossView.size()) return;
    ...
    html += QString("<span style='background-color:%1; color:%2; padding:2px 8px;"
                    " border-radius:6px; margin-left:10px; font-weight:bold;'>%3</span><br>")
              .arg(severityBg(f.severity), severityColor(f.severity),
                   f.severity.toUpper());
    html += QString("<span style='color:#666;'>Visibility:</span> <b>%1</b><br>")
                .arg(f.visibleIn);
    html += QString("<br>%1").arg(f.reason.toHtmlEscaped());

    html += "<br><br><i style='color:#888; font-size:11px;'>Note: a single "
...
void SystemStatusPanel::onExtensionRowClicked(QListWidgetItem* item)
{
    if (!item) return;
    const int idx = item->data(Qt::UserRole).toInt();
    if (idx < 0 || idx >= m_snapshot.rootkit.extensions.size()) return;
    ...
    html += QString("<span style='background-color:%1; color:%2; padding:2px 8px;"
                    " border-radius:6px; margin-left:10px; font-weight:bold;'>%3</span><br>")
              .arg(severityBg(k.severity), severityColor(k.severity),
                   k.severity.toUpper());
    html += QString("<span style='color:#666;'>Source:</span> %1 "
                    "(%2)<br>")
                .arg(k.source)
                .arg(k.isUserspace ? "user-space" : "kernel-resident");
    if (!k.name.isEmpty() && k.name != k.bundleId)
        html += QString("<span style='color:#666;'>Name:</span> %1<br>")
                    .arg(k.name.toHtmlEscaped());
    if (!k.version.isEmpty())
        html += QString("<span style='color:#666;'>Version:</span> %1<br>")
                    .arg(k.version);
    if (!k.teamId.isEmpty())
        html += QString("<span style='color:#666;'>Team ID:</span> "
                        "<code>%1</code><br>").arg(k.teamId);
    if (!k.signedBy.isEmpty())
        html += QString("<span style='color:#666;'>Signed by:</span> %1<br>")
                    .arg(k.signedBy.toHtmlEscaped());
    if (!k.state.isEmpty())
        html += QString("<span style='color:#666;'>State:</span> %1<br>")
                    .arg(k.state.toHtmlEscaped());
    if (!k.notes.isEmpty()) {
        html += "<br><b>Notes:</b><ul style='margin-top:4px;'>";
...
void SystemStatusPanel::onIntegrityRowClicked(QListWidgetItem* item)
{
    if (!item) return;
    const int idx = item->data(Qt::UserRole).toInt();
    if (idx < 0 || idx >= m_snapshot.rootkit.integrity.size()) return;
    ...
    if (!f.expectedHash.isEmpty()) {
        html += QString("<br><span style='color:#666;'>Baseline SHA-256:</span><br>"
                        "<code style='font-size:11px;'>%1</code><br>")
                    .arg(f.expectedHash);
    }
    if (!f.currentHash.isEmpty()) {
        html += QString("<span style='color:#666;'>Current SHA-256:</span><br>"
                        "<code style='font-size:11px;'>%1</code><br>")
                    .arg(f.currentHash);
    }
    if (f.currentSize > 0)
        html += QString("<span style='color:#666;'>Size:</span> %1 bytes<br>")
                    .arg(f.currentSize);

    if (f.status == "mismatch") {
        html += "<br><i style='color:#C62828; font-size:12px;'>"
