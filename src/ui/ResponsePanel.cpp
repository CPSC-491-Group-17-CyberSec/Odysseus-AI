...
// Alert detail panel.
// ---------------------------------------------------------------------------
std::vector<ActionButton> buildAlertActions(const ResponseManager& mgr, const ActionTarget& t) {
  ...
  if (t.kind == TargetKind::File || t.kind == TargetKind::Persistence) {
    out.push_back(make(
        mgr,
        t,
        ActionType::OpenLocation,
        "Open Location",
  ...
  if (t.kind == TargetKind::Integrity) {
    out.push_back(make(
        mgr,
        t,
        ActionType::ResetIntegrityBaseline,
        "Reset Baseline",
...
// ---------------------------------------------------------------------------
std::vector<ActionButton> buildResultActions(const ResponseManager& mgr, const ActionTarget& t) {
  std::vector<ActionButton> out;
  out.push_back(make(
      mgr,
      t,
      ...
      false));
  out.push_back(
      make(mgr, t, ActionType::CopyPath, "Copy Path", "Copy the absolute file path.", false));
  out.push_back(
      make(mgr, t, ActionType::CopyHash, "Copy SHA-256", "Copy the SHA-256 hash.", false));
  out.push_back(make(
      mgr,
      t,
      ActionType::QuarantineFile,
...
std::vector<ActionButton> buildSystemStatusActions(
    const ResponseManager& mgr, const ActionTarget& t) {
  std::vector<ActionButton> out;
  ...
  if (t.kind == TargetKind::Process) {
    ...
    out.push_back(make(
        mgr,
        t,
        ActionType::DisablePersistenceItem,
        "Disable",
    ...
    out.push_back(make(
        mgr,
        t,
        ActionType::MarkTrustedAfterReview,
        "Mark Trusted",
...
ConfirmPrompt confirmPromptFor(ActionType action, const ActionTarget& t) {
  ...
  switch (action) {
    case ActionType::QuarantineFile:
      ...
      p.body =
          "Odysseus will move this file to the app quarantine "
    ...
    case ActionType::RestoreFromQuarantine:
      ...
      p.body =
          "Odysseus will move this file back to its original "
          "location.";
      break;
    ...
    case ActionType::KillProcess:
      ...
      p.body =
          "Killing a process can cause data loss or system "
    ...
    case ActionType::DisablePersistenceItem:
      ...
      p.body =
          "Odysseus will mark this item as disabled in its "
    ...
    case ActionType::ResetIntegrityBaseline:
      ...
      p.body =
          "The stored baseline will be replaced with the current "
    ...
    case ActionType::MarkTrustedAfterReview:
      ...
      p.body =
          "Confirm that you reviewed the finding and accept it as "
          "trustworthy.";
      break;
