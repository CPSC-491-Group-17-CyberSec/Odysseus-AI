...
MainWindow::MainWindow(QWidget* parent)
    ...
    qInfo().noquote() << "[Response] subsystem ready —"
                      ...
                      << QString::fromStdString(
                             mgr.allowlist().list().empty()
                                 ? std::string("(empty)")
                                 : std::to_string(mgr.allowlist().list().size()) + " entries")
                      << "| quarantine dir:"
                      << QString::fromStdString(mgr.quarantine().quarantineDir())
                      << "| audit log:" << QString::fromStdString(mgr.actionLog().filePath());
  }

  setupUi();

  // Existing connections
  connect(runScanButton, &QPushButton::clicked, this, &MainWindow::onRunScanClicked);
  connect(historyButton, &QPushButton::clicked, this, &MainWindow::onHistoryClicked);
  connect(systemStatusButton, &QPushButton::clicked, this, &MainWindow::onSystemStatusClicked);
  connect(searchInput, &QLineEdit::textChanged, this, &MainWindow::onFilterOrSearchChanged);
  connect(
      severityFilter, &QComboBox::currentTextChanged, this, &MainWindow::onFilterOrSearchChanged);

  // Scan-type overlay (must be created after setupUi so the window has a size)
  m_scanOverlay = new ScanTypeOverlay(this);
  connect(
      m_scanOverlay, &ScanTypeOverlay::fullScanRequested, this, &MainWindow::onFullScanRequested);
  connect(
      m_scanOverlay,
      &ScanTypeOverlay::partialScanRequested,
      this,
      &MainWindow::onPartialScanRequested);
  connect(
      m_scanOverlay,
      &ScanTypeOverlay::resumeScanRequested,
      this,
      &MainWindow::onResumeScanRequested);

  // Scanner
  m_scanner = new FileScanner(this);
  connect(m_scanner, &FileScanner::scanningPath, this, &MainWindow::onScanningPath);
  connect(m_scanner, &FileScanner::progressUpdated, this, &MainWindow::onProgressUpdated);
  connect(m_scanner, &FileScanner::suspiciousFileFound, this, &MainWindow::onSuspiciousFileFound);
  connect(m_scanner, &FileScanner::scanFinished, this, &MainWindow::onScanFinished);
  connect(m_scanner, &FileScanner::scanError, this, &MainWindow::onScanError);
  ...
  connect(m_nam, &QNetworkAccessManager::finished, this, &MainWindow::onCveLookupReply);

  // Database
  m_db = new ScanDatabase(this);
  connect(m_db, &ScanDatabase::recordSaved, this, &MainWindow::onDbRecordSaved);
  connect(m_db, &ScanDatabase::databaseError, this, [](const QString& msg) {
    qWarning() << "[DB ERROR]" << msg;
  });

  // Wire scanner cache signal → DB flush
  connect(m_scanner, &FileScanner::cacheUpdateReady, this, &MainWindow::onCacheUpdateReady);
  ...
  for (const ScanRecord& r : m_history) {
    ...
    QStringList cats;
    if (c > 0)
      cats << QString("%1 crit").arg(c);
    if (s > 0)
      cats << QString("%1 susp").arg(s);
    if (rv > 0)
      cats << QString("%1 review").arg(rv);
    QString buckets;
...
void MainWindow::setupShell() {
  ...
  connect(m_sidebar, &Sidebar::pageRequested, this, &MainWindow::onSidebarPageRequested);
  shellLayout->addWidget(m_sidebar);

  // Page stack
  m_pageStack = new QStackedWidget(shell);
  m_pageStack->setStyleSheet(
      QString("QStackedWidget { background-color: %1; }").arg(Theme::Color::bgPrimary));

  // Page 0 — DASHBOARD: the new Phase 4 redesigned dashboard.
  m_dashboardPage = new DashboardPage(this);
  m_pageStack->insertWidget(PageDashboard, m_dashboardPage);
  connect(
      m_dashboardPage, &DashboardPage::scanRequested, this, &MainWindow::onDashboardScanRequested);
  connect(
      m_dashboardPage,
      &DashboardPage::viewAllActivityClicked,
      this,
      &MainWindow::onDashboardViewAllActivity);
  ...
  connect(m_scanPage, &ScanPage::scanRequested, this, &MainWindow::onScanPageStartRequested);
  connect(m_scanPage, &ScanPage::exportLogsRequested, this, &MainWindow::onScanPageExportLogs);
  connect(
      m_scanPage, &ScanPage::viewAllRecentRequested, this, &MainWindow::onScanPageViewAllRecent);
  ...
  connect(m_resultsPage, &ResultsPage::exportRequested, this, []() {
  ...
  if (m_systemPanel) {
    ...
    m_pageStack->insertWidget(
        PageSystemStatus, makePlaceholderPage("System Status", "System monitor not initialized."));
  }
  ...
  m_pageStack->insertWidget(
      PageThreatIntel,
      makePlaceholderPage("Threat Intelligence", "Reputation database explorer — coming soon."));
  m_pageStack->insertWidget(
      PageReports, makePlaceholderPage("Reports", "Export & report history — coming soon."));
  ...
  connect(
      m_threatDetail,
      &ThreatDetailPanel::closeRequested,
      this,
      &MainWindow::onThreatDetailCloseRequested);

  auto* splitter = new QSplitter(Qt::Horizontal, shell);
  splitter->setHandleWidth(2);
  splitter->setStyleSheet(QString("QSplitter::handle { background-color: %1; }"
  ...
  connect(m_monitor, &MonitoringService::alertRaised, this, &MainWindow::onEdrAlertRaised);
  connect(m_monitor, &MonitoringService::tickCompleted, this, &MainWindow::onEdrTickCompleted);
  connect(
      m_monitor, &MonitoringService::monitoringStateChanged, this, &MainWindow::onEdrStateChanged);

  // Dedup engine — alertResolved + alertUpdated forward straight to
  // the Alerts page (and the Dashboard's score recompute).
  connect(m_monitor, &MonitoringService::alertResolved, this, [this](const EDR::Alert& a) {
    if (m_alertsPage)
      m_alertsPage->markAlertResolved(a);
    refreshDashboardScore();
  });
  connect(m_monitor, &MonitoringService::alertUpdated, this, [this](const EDR::Alert& a) {
    if (m_alertsPage)
      m_alertsPage->updateAlert(a);
  ...
  if (m_scanPage) {
    if (!m_history.isEmpty()) {
      ...
      const int totalThreats = latest.criticalCount + latest.suspiciousOnly + latest.reviewCount;
      m_scanPage->setStats(
          latest.timestamp,
          latest.totalScanned,
          totalThreats,
...
void MainWindow::retireLegacyHeader() {
  ...
  if (m_legacyDashWrap) {
    if (auto* mainLayout = qobject_cast<QVBoxLayout*>(m_legacyDashWrap->layout())) {
  ...
  if (m_legacyDashWrap) {
    m_legacyDashWrap->setStyleSheet(
  ...
  if (searchInput) {
    ...
    searchInput->setStyleSheet(QString("QLineEdit {"
                                       "  background-color: %1; color: %2;"
  ...
  // ── Severity filter ────────────────────────────────────────────────
  if (severityFilter) {
    severityFilter->setStyleSheet(QString("QComboBox {"
                                          "  background-color: %1; color: %2;"
                                          "  border: 1px solid %3; border-radius: 8px;"
  ...
  if (scanResultsPanel)
    scanResultsPanel->setStyleSheet(panelQss);
  if (historyPanel)
    historyPanel->setStyleSheet(panelQss);
  if (historyDetailPanel)
    historyDetailPanel->setStyleSheet(panelQss);

  if (scanResultsList)
    scanResultsList->setStyleSheet(listQss);
  if (historyList)
    historyList->setStyleSheet(listQss);
  if (histDetailFilesList)
    histDetailFilesList->setStyleSheet(listQss);

  // Inline labels inside the panels — make sure they read against dark.
  auto reskinLabel = [](QLabel* l, int size, int weight) {
    if (!l)
      return;
    l->setStyleSheet(QString("QLabel { color: %1; %2 background: transparent; }")
                         .arg(Theme::Color::textPrimary, Theme::Type::qss(size, weight)));
  };
...
QWidget* MainWindow::makePlaceholderPage(const QString& title, const QString& subtitle) {
  ...
  page->setStyleSheet(QString("background-color: %1;").arg(Theme::Color::bgPrimary));
  ...
  h->setStyleSheet(
      QString("color: %1; font-size: 28px; font-weight: 700;").arg(Theme::Color::textPrimary));
  v->addWidget(h);

  auto* sub = new QLabel(subtitle, page);
  sub->setWordWrap(true);
  sub->setStyleSheet(
      QString("color: %1; font-size: 14px; line-height: 1.5;").arg(Theme::Color::textSecondary));
  v->addWidget(sub);
  ...
  pill->setStyleSheet(QString("QLabel { background-color: %1; color: white;"
...
void MainWindow::onSidebarPageRequested(int index) {
  ...
  if (index == PageSettings) {
    if (auto* sp = qobject_cast<SettingsPage*>(m_pageStack->widget(PageSettings))) {
      sp->reloadFromConfig();
    }
  }
}
...
// ============================================================================
void MainWindow::onScanPageStartRequested(const QStringList& targets, int /*depth*/) {
  ...
  if (targets.size() > 1) {
    qInfo().noquote() << QString(
                             "[Scan] %1 targets selected — only the first ('%2') "
...
void MainWindow::onEdrAlertRaised(const EDR::Alert& alert) {
  ...
  if (m_alertsPage)
    m_alertsPage->appendAlert(alert);

  // Surface it on the dashboard's Recent Activity feed too
  if (m_dashboardPage)
    m_dashboardPage->appendEdrAlert(alert);
...
void MainWindow::onThreatDetailCloseRequested() {
  ...
  anim->setEndValue(0);
  connect(anim, &QVariantAnimation::valueChanged, splitter, [splitter, this](const QVariant& v) {
    const int w = v.toInt();
...
void MainWindow::setupUi() {
  ...
  auto makeStat = [&](const QString& label, const QString& color, QLabel*& valueLabel) {
    ...
    auto* dot = new QLabel(
        QString("<span style='color:%1; font-size:16px;'>\xe2\x97\x8f</span>").arg(color));
  ...
  scoreBarBg->setStyleSheet("QFrame { background-color: #D0D0D0; border-radius: 5px; }");
  // Score bar fill (overlaid)
  aiScoreFillBar = new QFrame(scoreBarBg);
  aiScoreFillBar->setGeometry(0, 0, 0, 10);
  aiScoreFillBar->setStyleSheet("QFrame { background-color: #2E7D32; border-radius: 5px; }");
  ...
  severityFilter->addItems(
      {"All Severities", "Critical", "Suspicious", "Needs Review", "High", "Medium", "Low"});
  ...
  threatTable->setHorizontalHeaderLabels(
      {"Severity", "Name", "AI Classification", "Confidence", "Vendor", "Published", "Status"});
  ...
  closeDetailsButton->setStyleSheet(
      "QPushButton { background-color: #E0E0E0; border-radius: 15px; font-weight: bold; border: "
      "none; }"
  ...
  connect(closeDetailsButton, &QPushButton::clicked, this, &MainWindow::onCloseDetailsClicked);
  ...
  connect(closeScanButton, &QPushButton::clicked, this, &MainWindow::onCloseScanResultsClicked);
  ...
  connect(closeHistoryButton, &QPushButton::clicked, this, &MainWindow::onCloseHistoryClicked);
  ...
  histDetailSummaryLabel->setStyleSheet(
      "font-size: 12px; color: #444; padding: 8px;"
  ...
  connect(
      closeHistoryDetailButton, &QPushButton::clicked, this, &MainWindow::onCloseHistoryClicked);
  connect(backToHistBtn, &QPushButton::clicked, this, &MainWindow::onHistoryClicked);
  ...
  connect(
      m_systemPanel,
      &SystemStatusPanel::refreshRequested,
      this,
      &MainWindow::onSystemRefreshRequested);
  connect(
      m_systemPanel, &SystemStatusPanel::closeRequested, this, &MainWindow::onSystemCloseRequested);
  connect(m_sysmon, &SystemMonitor::snapshotReady, this, &MainWindow::onSystemSnapshotReady);
  connect(m_sysmon, &SystemMonitor::snapshotError, this, &MainWindow::onSystemSnapshotError);
}
...
// ============================================================================
void MainWindow::addThreatEntry(
...
void MainWindow::addScanFindingToTable(const SuspiciousFile& sf) {
  ...
  auto scoreStr = [&]() -> QString {
    return (sf.cveScore > 0.0f) ? QString(" (%1)").arg(double(sf.cveScore), 0, 'f', 1) : QString();
  };

  if (sf.cveSeverity == "CRITICAL") {
    sevText = "Critical" + scoreStr();
    sevColor = QColor("#C62828");
  } else if (sf.cveSeverity == "HIGH") {
    sevText = "High" + scoreStr();
    sevColor = QColor("#E65100");
  } else if (sf.cveSeverity == "MEDIUM") {
    sevText = "Medium" + scoreStr();
    sevColor = QColor("#F57F17");
  } else if (sf.cveSeverity == "LOW") {
    sevText = "Low" + scoreStr();
    sevColor = QColor("#2E7D32");
    ...
    if (sf.category.contains("Known Malware") || sf.category.contains("PE Binary") ||
      ...
      sevColor = QColor("#C62828");
        ...
        sf.category.contains("High-Risk") || sf.category.contains("Persistence") ||
  ...
  if (!sf.classificationLevel.isEmpty()) {
    QString cls = sf.classificationLevel.toUpper();
    if (cls == "CRITICAL") {
      aiClassText = "CRITICAL";
      aiClassColor = QColor("#C62828");
    } else if (cls == "SUSPICIOUS") {
      aiClassText = "Suspicious";
      aiClassColor = QColor("#E65100");
    } else if (cls == "ANOMALOUS") {
      aiClassText = "Anomalous";
      aiClassColor = QColor("#F57F17");
    } else if (cls == "CLEAN") {
      aiClassText = "Clean";
      aiClassColor = QColor("#2E7D32");
    } else {
      aiClassText = sf.classificationLevel;
    }
  }
  ...
  if (sf.anomalyScore > 0.0f) {
    ...
    if (sf.anomalyScore >= 0.8f)
      confColor = QColor("#C62828");
    else if (sf.anomalyScore >= 0.6f)
      confColor = QColor("#E65100");
    else if (sf.anomalyScore >= 0.4f)
      confColor = QColor("#F57F17");
    else
      confColor = QColor("#2E7D32");
  }
  ...
  QList<QColor> colColors = {
      sevColor, darkText, aiClassColor, confColor, darkText, darkText, darkText};
  for (int i = 0; i < cols.size(); ++i) {
    auto* item = new QTableWidgetItem(cols[i]);
...
void MainWindow::lookupCveForFinding(int idx) {
  ...
  req.setHeader(QNetworkRequest::UserAgentHeader, "Odysseus-Dashboard/1.0 (contact@example.com)");
...
void MainWindow::onThreatDoubleClicked(int row, int /*column*/) {
  ...
  if (!filePath.isEmpty()) {
    ...
    if (findingIdx >= 0 && (!m_findings[findingIdx].classificationLevel.isEmpty() ||
      ...
      if (m_threatDetail) {
        ...
        if (!m_threatDetail->isVisible()) {
          ...
          if (auto* splitter = qobject_cast<QSplitter*>(m_threatDetail->parentWidget())) {
            ...
            connect(
                ...
                splitter,
                [splitter, this](const QVariant& v) {
              const int w = v.toInt();
              splitter->setSizes({width() - w, w});
            });
            anim->start(QAbstractAnimation::DeleteWhenStopped);
      ...
      QString displayLevel =
          !sf.classificationLevel.isEmpty() ? sf.classificationLevel : sf.severityLevel;
      QString levelColor, levelBg;
      QString levelUpper = displayLevel.toUpper();
      if (levelUpper == "CRITICAL") {
        levelColor = "#C62828";
        levelBg = "#FDECEA";
      } else if (levelUpper == "SUSPICIOUS") {
        levelColor = "#E65100";
        levelBg = "#FFF3E0";
      } else if (levelUpper == "HIGH") {
        levelColor = "#E65100";
        levelBg = "#FFF3E0";
      } else if (levelUpper == "ANOMALOUS") {
        levelColor = "#F57F17";
        levelBg = "#FFFDE7";
      ...
      QString barColor = (barPct >= 80)   ? "#C62828"
                         : (barPct >= 60) ? "#E65100"
                         : (barPct >= 40) ? "#F57F17"
                                          : "#2E7D32";
      ...
      descHtml +=
          "<div style='background-color:#FAFAFA; border-radius:10px; padding:14px;"
      ...
      descHtml += QString(
                      ...
                      .arg(
                          ...
                          displayLevel,
                          barColor,
                          QString::number(double(sf.anomalyScore), 'f', 3));
      // Score bar
      descHtml += QString(
      ...
      // File metadata
      descHtml += "<span style='font-size:11px; color:#777; word-wrap:break-word;'>" + displayPath +
                  "</span><br>";
      descHtml += "<span style='font-size:12px; color:#555;'>" + QString::number(sf.sizeBytes) +
                  " bytes &nbsp;|&nbsp; " + sf.lastModified.toString("yyyy-MM-dd hh:mm:ss") +
                  "</span>";
      ...
      const bool hasReputation = !sf.reputationFamily.isEmpty() || sf.reputationPrevalence > 0;
      const bool hasSigning = (sf.signingStatus >= 0);
      const bool hasConfidence = (sf.confidencePct > 0.0f);

      if (hasYara || hasReputation || hasSigning || hasConfidence || !sf.sha256.isEmpty()) {
        QString tiHtml;
        tiHtml +=
            "<div style='background-color:#F0F4F8; border-radius:8px;"
            " padding:10px 14px; margin:8px 0 0 0;"
            " border-left:4px solid #1976D2;'>";
        tiHtml +=
            "<span style='font-size:13px; font-weight:bold; color:#0D47A1;'>"
        ...
        if (hasYara) {
          tiHtml +=
              "<span style='font-size:12px; color:#555;'>"
              "YARA: <b>" +
              sf.yaraMatches.join(", ") + "</b>";
          if (!sf.yaraFamily.isEmpty())
            tiHtml += " &nbsp;\xE2\x80\x94 family <b>" + sf.yaraFamily + "</b>";
        ...
        if (hasReputation) {
          ...
          if (!sf.reputationSource.isEmpty())
            tiHtml += QString(" &nbsp;(source: %1)").arg(sf.reputationSource);
        ...
        if (hasSigning) {
          ...
          switch (sf.signingStatus) {
            case 2:
              signText = "signed (trusted)";
              signColor = "#2E7D32";
              break;
            case 1:
              signText = "signed (untrusted)";
              signColor = "#F57F17";
              break;
            case 0:
              signText = "UNSIGNED";
      ...
      if (!sf.keyIndicators.isEmpty()) {
        aiHtml +=
            "<div style='background-color:#FFF3E0; border-radius:8px; padding:10px 14px;"
        ...
        for (const QString& ind : sf.keyIndicators) {
          if (count >= 3)
            break;
          aiHtml += "<span style='font-size:12px; color:#444;'>\xe2\x80\xa2 " + ind + "</span><br>";
          ++count;
        }
        aiHtml += "</div>";
      }
      ...
      // ════════════════════════════════════════════════════════════
      aiHtml +=
          "<div style='background-color:#EDE7F6; border-radius:10px; padding:14px;"
          " margin:0 0 4px 0; border-left:4px solid #5C6BC0;'>";
      aiHtml +=
          "<span style='font-size:14px; font-weight:bold; color:#1A237E;'>"
      ...
      if (!sf.aiSummary.isEmpty()) {
        aiHtml += "<span style='font-size:12px; color:#333; line-height:1.5;'>" + sf.aiSummary +
                  "</span><br>";
      } else {
        aiHtml +=
      ...
      if (sf.keyIndicators.size() > 3) {
        aiHtml +=
            "<br><span style='font-size:11px; font-weight:bold; color:#555;'>"
            "All indicators:</span><br>";
        for (const QString& ind : sf.keyIndicators) {
          aiHtml += "<span style='font-size:11px; color:#555;'>\xe2\x96\xb8 " + ind + "</span><br>";
        }
      }

      aiHtml +=
          "<br><span style='font-size:10px; color:#388E3C; font-weight:bold;'>"
      ...
      aiHtml +=
          "<div style='background-color:#F5F9FF; border-radius:8px; padding:12px;"
      ...
      if (!sf.aiExplanation.isEmpty()) {
        ...
        aiHtml +=
            "<br><span style='font-size:10px; color:#1565C0; font-weight:bold;'>"
        ...
        aiHtml +=
            "<br><span style='font-size:12px; color:#1565C0; font-style:italic;'>"
            "\xe2\x8f\xb3 Generating LLM explanation...</span><br>";
        aiHtml +=
            "<br><span style='font-size:10px; color:#90CAF9;'>"
            "LLM: Processing</span>";
      } else {
        ...
        aiHtml +=
            "<br><span style='font-size:10px; color:#B0B0B0;'>"
      ...
      if (!sf.recommendedActions.isEmpty()) {
        actHtml +=
            "<div style='background-color:#FFF8E1; border-radius:8px; padding:10px;"
            " margin:4px 0; border-left:4px solid #F57F17;'>";
        actHtml +=
            "<b style='font-size:13px;'>Recommended Actions</b>"
    ...
    detailsDescLabel->setText(
        "<b>Description:</b><br>"
        "A vulnerability or active threat associated with " +
        vendor +
        ". "
...
void MainWindow::startScanForPath(const QString& rootPath) {
  ...
  aiStatsModelLabel->setStyleSheet("font-size: 11px; color: #388E3C; font-style: italic;");
  aiStatsLlmLabel->setText(
      "LLM Explanation: Ollama / Llama3  \xe2\x80\xa2  Waiting for findings...");
...
void MainWindow::onSuspiciousFileFound(const SuspiciousFile& file) {
  ...
  if (!file.classificationLevel.isEmpty()) {
    ...
    if (cls == "CRITICAL")
      ...
      logLabel = "[NEEDS REVIEW]";
  }
  qDebug().noquote() << logLabel << file.category << "|" << file.filePath;
  ...
  if (!file.classificationLevel.isEmpty()) {
    ...
    if (file.category.contains("Known Malware") || file.category.contains("PE Binary") ||
      ...
      bg = QColor("#FDECEA");
        ...
        file.category.contains("High-Risk") || file.category.contains("Persistence") ||
        file.category.contains("Temp")) {
      prefix = "[HIGH]";
      bg = QColor("#FFF3E0");
    } else if (
        file.category.contains("Suspicious Name") || file.category.contains("Double-Extension")) {
      prefix = "[MED]";
      bg = QColor("#FFFDE7");
    }
  }
  ...
  QString displayText =
      QString("%1 %2%3\n   %4\n   %5")
          ...
          .arg(scoreInfo)
          .arg(file.category)
          .arg(file.filePath.length() > 60 ? "..." + file.filePath.right(57) : file.filePath);
  ...
  if (tally.isEmpty())
    tally = "0";
  scanSummaryLabel->setText(QString("Findings so far: %1").arg(tally));
  ...
  if (scoreN > 0) {
    ...
    // Color-code average
    QString avgColor = (avg >= 0.8f)   ? "#C62828"
                       : (avg >= 0.6f) ? "#E65100"
                       : (avg >= 0.4f) ? "#F57F17"
                                       : "#2E7D32";
  ...
  if (anyLlm) {
    ...
    aiStatsLlmLabel->setText(
        "LLM Explanation: Ollama / Llama3  \xe2\x80\xa2  Connected (no output yet)");
    aiStatsLlmLabel->setStyleSheet("font-size: 11px; color: #888; font-style: italic;");
  } else {
...
void MainWindow::onScanFinished(
  ...
  if (m_findings.isEmpty()) {
    ...
    QStringList parts;
    if (nCrit > 0)
      parts << QString("%1 critical").arg(nCrit);
    if (nSusp > 0)
      parts << QString("%1 suspicious").arg(nSusp);
    if (nReview > 0)
      parts << QString("%1 needs review").arg(nReview);
    ...
    if (cveCount > 0) {
      scanSummaryLabel->setText(QString("Looking up CVEs for %1 finding(s)...").arg(cveCount));
    } else {
      scanSummaryLabel->setText(
...
void MainWindow::onHistoryClicked() {
  ...
  if (m_history.isEmpty()) {
    ...
    for (const ScanRecord& r : m_history) {
      ...
      QStringList cats;
      if (crit > 0)
        cats << QString("%1 crit").arg(crit);
      if (susp > 0)
        cats << QString("%1 susp").arg(susp);
      if (rev > 0)
        cats << QString("%1 review").arg(rev);
      ...
      if (crit > 0)
        color = QColor("#B71C1C");       // red
      else if (susp > 0)
        color = QColor("#E65100");       // orange
      else if (rev > 0)
        color = QColor("#F57F17");       // amber
      else if (r.suspiciousCount > 0)
        color = QColor("#B71C1C");       // legacy fallback
...
void MainWindow::onSystemCloseRequested() {
  ...
  if (m_pageStack)
    m_pageStack->setCurrentIndex(PageDashboard);
  if (m_sidebar)
    m_sidebar->setActive(PageDashboard);
}

void MainWindow::onSystemSnapshotReady(const SystemSnapshot& snap) {
  if (m_systemPanel)
    m_systemPanel->setSnapshot(snap);
...
void MainWindow::onSystemSnapshotError(const QString& message) {
  ...
  if (m_systemPanel)
    m_systemPanel->setRefreshing(false);
}

void MainWindow::showHistoryDetail(const ScanRecord& record) {
  histDetailTitleLabel->setText("Scan — " + record.timestamp.toString("yyyy-MM-dd hh:mm:ss"));
  ...
  QStringList buckets;
  if (dCrit > 0)
    buckets << QString("%1 critical").arg(dCrit);
  if (dSusp > 0)
    buckets << QString("%1 suspicious").arg(dSusp);
  if (dRev > 0)
    buckets << QString("%1 needs review").arg(dRev);
  QString bucketStr;
  ...
  histDetailSummaryLabel->setText(QString("Files scanned: %1\n"
                                          "Findings: %2\n"
                                          "Duration: %3")
  ...
  if (record.findings.isEmpty()) {
    ...
    for (const SuspiciousFile& sf : record.findings) {
      QString cveTag =
          sf.cveId.isEmpty() ? "" : QString("  [%1 – %2]").arg(sf.cveId, sf.cveSeverity);
      ...
      QString text =
          QString("• %1%2\n   %3\n   %4")
              ...
              .arg(cveTag)
              .arg(sf.category)
              .arg(sf.filePath.length() > 65 ? "…" + sf.filePath.right(62) : sf.filePath);
...
void MainWindow::refreshDetailLlmSection(const SuspiciousFile& sf) {
  ...
  if (!sf.aiExplanation.isEmpty()) {
    aiHtml += "<br><span style='font-size:12px; color:#333; line-height:1.5;'>" + sf.aiExplanation +
              "</span><br>";
    aiHtml +=
        "<br><span style='font-size:10px; color:#1565C0; font-weight:bold;'>"
        "\xe2\x9c\x93 LLM: Active</span>";
