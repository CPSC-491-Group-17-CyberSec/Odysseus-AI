...
const char* severityHex(const QString& sev)
{
    ...
    if (s == "critical") return Color::severityCritical;
    if (s == "high")      return Color::severityHigh;
    if (s == "medium")    return Color::severityMedium;
    if (s == "low")        return Color::severityLow;
    if (s == "safe" || s == "ok" || s == "clean") return Color::severitySafe;
