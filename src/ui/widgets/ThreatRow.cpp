...
const char* severityHex(const QString& sev)
{
    ...
    if (s == "clean" || s == "safe") return Theme::Color::severitySafe;
    ...
    : QFrame(parent)
{
    ...
    m_dot->setStyleSheet(QString(
        "QLabel { background-color: %1; border-radius: 5px; }")
    ...
    m_subtext->setStyleSheet(QString(
        "QLabel { color: %1; %2 background: transparent; }")
    ...
    m_confidenceNum->setStyleSheet(QString(
        "QLabel { color: %1; %2 background: transparent;"
    ...
    m_confidenceBar->setStyleSheet(QString(
        "QFrame { background-color: %1; border-radius: 2px; }")
    ...
    m_confidenceFill->setStyleSheet(QString(
        "QFrame { background-color: %1; border-radius: 2px; }")
    ...
    m_source->setStyleSheet(QString(
        "QLabel { color: %1; %2 background: transparent; }")
            .arg(Theme::Color::textSecondary)
            .arg(Theme::Type::qss(Theme::Type::Small)));
    ...
    m_detected->setStyleSheet(QString(
        "QLabel { color: %1; %2 background: transparent; }")
            .arg(Theme::Color::textSecondary)
            .arg(Theme::Type::qss(Theme::Type::Small)));
...
void ThreatRow::setSeverity(const QString& severity)
{
    ...
    m_dot->setStyleSheet(QString(
        "QLabel { background-color: %1; border-radius: 5px; }").arg(hex));
    ...
    m_confidenceFill->setStyleSheet(QString(
        "QFrame { background-color: %1; border-radius: 2px; }").arg(hex));
}
...
void ThreatRow::setConfidence(float zeroToOne)
{
    ...
    const int trackWidth = m_confidenceBar->width() > 0
                              ? m_confidenceBar->width()
                              : 120;
    m_confidenceFill->setFixedWidth(static_cast<int>(trackWidth * clamped));
}

void ThreatRow::setSource(const QString& s)    { m_source->setText(s); }
void ThreatRow::setDetected(const QString& s)  { m_detected->setText(s); }
void ThreatRow::setStatus(const QString& s)    { m_statusBadge->setText(s); }
