...
{
    switch (t) {
        case ActivityList::Critical: return "\xE2\x9A\xA0";   // ⚠ threat detected
        case ActivityList::Warning:  return "\xE2\x9A\xA0";   // ⚠ suspicious
        case ActivityList::Info:     return "\xE2\x96\xB6";   // ▶ scan completed
        case ActivityList::Success:  return "\xE2\x9C\x93";   // ✓ all clear
        case ActivityList::System:   return "\xE2\x97\x8E";   // ◎ system event
    }
    return "?";
}
...
{
    switch (t) {
        case ActivityList::Critical: return Theme::Color::severityCritical;
        case ActivityList::Warning:  return Theme::Color::severityMedium;
        case ActivityList::Info:     return Theme::Color::accentBlue;
        case ActivityList::Success:  return Theme::Color::severitySafe;
        case ActivityList::System:   return "#A78BFA";   // soft purple
    }
...
QString relativeTime(const QDateTime& when)
{
    ...
    if (secs < 60)         return QString("%1 sec ago").arg(secs);
    if (secs < 60*60)      return QString("%1 min ago").arg(secs / 60);
    if (secs < 60*60*24)   return QString("%1 hour ago").arg(secs / 3600);
    ...
    : QFrame(parent)
{
    ...
    viewAll->setStyleSheet(QString(
        "QPushButton { background: transparent; color: %1; font-size: 12px;"
        " font-weight: 600; padding: 4px 8px; border: none; }"
        "QPushButton:hover { color: %2; }"
    ).arg(Theme::Color::accentBlue, Theme::Color::accentBlueHover));
    connect(viewAll, &QPushButton::clicked,
            this, &ActivityList::viewAllClicked);
...
void ActivityList::setEntries(const QVector<Entry>& entries)
{
    ...
    for (const auto& e : entries) {
        ...
        iconWrap->setStyleSheet(QString(
            "QFrame { background-color: %1; border-radius: 8px; }")
        ...
        icon->setStyleSheet(QString(
            "QLabel { color: %1; font-size: 16px; font-weight: 700;"
