    ...
    : QWidget(parent)
{
    ...
    setStyleSheet(QString("QWidget#OdySidebar { background-color: %1; }")
                      .arg(Theme::Color::bgSidebar));
    ...
    brand->setStyleSheet(QString(
        ...
        " letter-spacing: 1px; padding: 0 4px; }").arg(Theme::Color::accentBlue));
    layout->addWidget(brand);

    auto* tagline = new QLabel("AI-Powered Cybersecurity", this);
    tagline->setStyleSheet(QString(
        "QLabel { color: %1; font-size: 10px; padding: 0 4px 18px 4px; }")
    ...
    m_footer->setStyleSheet(QString(
        "QLabel { color: %1; font-size: 10px; padding: 4px; }")
            .arg(Theme::Color::textMuted));
...
{
    if (m_footer) m_footer->setText(text);
}
