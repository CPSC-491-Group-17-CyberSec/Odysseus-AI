...
const char* AlertRow::iconForCategory(const QString& cat) {
  ...
  if (cat == EDR::Category::Process)
    return "\xE2\x96\xB6";
  if (cat == EDR::Category::Persistence)
    return "\xE2\x97\x8E";
  if (cat == EDR::Category::Integrity)
    return "\xE2\xAC\xA2";
  if (cat == EDR::Category::KernelExt)
    return "\xE2\x9B\xA8";
  if (cat == EDR::Category::CrossView)
    return "\xE2\x87\x84";
  return "\xE2\x97\x86";
}
...
QString AlertRow::smartTruncate(const QString& path, int maxLen) {
  ...
  if (path.contains('/') || path.contains('\\')) {
    ...
    if (parts.size() >= 3) {
      ...
      QString shortened = prefix + head + sep + "..." + sep + tailParent + sep + tail;
...
AlertRow::AlertRow(QWidget* parent)
  ...
  m_dot->setStyleSheet(QString("QLabel { background-color: %1; border-radius: 4px; }")
                           .arg(Theme::Color::textSecondary));
  h->addWidget(m_dot, 0, Qt::AlignVCenter);

  // ── Time ───────────────────────────────────────────────────────────
  m_timeLab = new QLabel("—", this);
  m_timeLab->setStyleSheet(QString("QLabel { color: %1; %2 background: transparent; }")
  ...
  m_categoryIcon->setStyleSheet(QString("QLabel { color: %1; %2 background: transparent; }")
                                    .arg(Theme::Color::textSecondary)
  ...
  m_sourceLab->setStyleSheet(QString("QLabel { color: %1; %2 background: transparent; }")
                                 .arg(Theme::Color::textSecondary)
                                 .arg(Theme::Type::qss(Theme::Type::Caption)));
...
//  Public API
// ============================================================================
void AlertRow::setAlert(
    const EDR::Alert& alert, int index, int occurrenceCount, bool isGroupHeader) {
  ...
  const QString dotHex =
      m_resolved ? QString(Theme::Color::textMuted) : QString(severityHexFor(alert.severity));
  m_dot->setStyleSheet(QString("QLabel { background-color: %1; border-radius: 4px; }").arg(dotHex));

  m_timeLab->setText(relTime(alert.lastSeen.isValid() ? alert.lastSeen : alert.timestamp));
...
void AlertRow::applyStyle() {
  QString bg = m_zebra ? Theme::Color::bgSecondary : Theme::Color::bgCard;
  QString left = "transparent";

  if (m_selected) {
    bg = Theme::Color::bgCardHover;
    left = m_resolved ? QString(Theme::Color::textMuted) : QString(severityHexFor(m_severity));
  } else if (m_hovered) {
    bg = Theme::Color::bgCardHover;
  }
  ...
  if (m_titleLab) {
    m_titleLab->setStyleSheet(
        QString("QLabel { color: %1; %2 background: transparent; }")
            .arg(m_resolved ? QString(Theme::Color::textMuted) : QString(Theme::Color::textPrimary))
            .arg(Theme::Type::qss(Theme::Type::Body, Theme::Type::WeightSemi)));
  }
  if (m_sourceLab) {
    m_sourceLab->setStyleSheet(QString("QLabel { color: %1; %2 background: transparent; }")
                                   .arg(
                                       m_resolved ? QString(Theme::Color::textMuted)
                                                  : QString(Theme::Color::textSecondary))
                                   .arg(Theme::Type::qss(Theme::Type::Caption)));
  }

  setStyleSheet(QString("QFrame#OdyAlertRow {"
                        "  background-color: %1;"
                        "  border: none;"
