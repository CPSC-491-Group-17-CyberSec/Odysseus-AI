    ...
    : QFrame(parent)
{
    ...
    m_cards[0] = buildCard(Quick,  QString::fromUtf8("\xE2\x9A\xA1"),     // ⚡
                            "Quick Scan",  "Scan critical areas",  "~ 5 min");
    m_cards[1] = buildCard(Full,   QString::fromUtf8("\xE2\x96\xA6"),     // ▦
                            "Full Scan",   "Deep system scan",      "~ 30-60 min");
    m_cards[2] = buildCard(Custom, QString::fromUtf8("\xE2\x9A\x99"),     // ⚙
                            "Custom Scan", "Choose what to scan",   "Custom");
    ...
    m_startBtn->setStyleSheet(QString(
        ...
        "QPushButton:hover { background-color: %2; }"
          ...
          Theme::Color::accentBlueHover));
    outer->addWidget(m_startBtn);

    connect(m_startBtn, &QPushButton::clicked,
            this, &ScanTypeSelector::onStartClicked);

    setSelected(Quick);
}

ScanTypeSelector::Card
ScanTypeSelector::buildCard(ScanType type, const QString& glyph,
                              ...
                              const QString& estimate)
{
    ...
    c.estimate->setStyleSheet(QString(
...
{
    const QString border = selected ? Theme::Color::accentBlue
                                     : Theme::Color::borderSubtle;
    const QString bg     = selected ? Theme::Color::accentBlueSoft
                                     : Theme::Color::bgPrimary;

    c.frame->setStyleSheet(QString(
        "QFrame#OdyScanTypeCard {"
        "  background-color: %1;"
        "  border: 2px solid %2;"
        "  border-radius: %3px;"
        "}"
    ).arg(bg, border).arg(Theme::Size::cardRadius));
}
...
void ScanTypeSelector::setSelected(ScanType type)
{
    ...
    QString label;
    switch (type) {
        case Quick:  label = QString::fromUtf8("\xE2\x96\xB6  Start Quick Scan"); break;
        case Full:   label = QString::fromUtf8("\xE2\x96\xB6  Start Full Scan");  break;
        case Custom: label = QString::fromUtf8("\xE2\x96\xB6  Start Custom Scan"); break;
    }
    if (m_startBtn) m_startBtn->setText(label);
}
