...
AlertDetailPanel::AlertDetailPanel(QWidget* parent)
  ...
  setMinimumWidth(380);
  setMaximumWidth(460);
  setStyleSheet(QString("QFrame#OdyAlertDetailPanel {"
...
void AlertDetailPanel::buildUi() {
  ...
  topRow->setSpacing(8);
  m_severity = new SeverityBadge(EDR::Severity::Info, SeverityBadge::Filled, this);
  ...
  m_timestampLab->setStyleSheet(QString("QLabel { color: %1; %2 background: transparent; }")
                                    .arg(Theme::Color::textMuted)
  ...
  m_titleLab->setStyleSheet(QString("QLabel { color: %1; %2 background: transparent; }")
  ...
  m_groupBanner->setStyleSheet(
      QString("QLabel { color: %1; background-color: %2;"
              " border: 1px solid %1; border-radius: 6px;"
              " padding: 6px 10px; %3 }")
          .arg(Theme::Color::accentBlue, Theme::Color::bgSecondary)
          .arg(Theme::Type::qss(Theme::Type::Caption, Theme::Type::WeightSemi)));
  ...
  m_tabs->setStyleSheet(QString("QTabWidget::pane { border: none; background: transparent;"
                                " padding-top: 8px; }"
                                "QTabBar::tab {"
  ...
  auto buildKv =
      [&](QVBoxLayout* host, const QString& key, QLabel*& valOut, bool monospace = false) {
    ...
    valOut->setStyleSheet(
        monospace ? QString("QLabel { color: %1; %2 background: transparent;"
  ...
  // ── Tab 2: Why Flagged ─────────────────────────────────────────────
  {
    ...
    m_whyEmpty->setStyleSheet(QString("QLabel { color: %1; %2 background: transparent;"
                                      " font-style: italic; }")
                                  .arg(Theme::Color::textMuted)
                                  .arg(Theme::Type::qss(Theme::Type::Body)));
  ...
  // ── Tab 3: Indicators ──────────────────────────────────────────────
  {
    ...
    m_indEmpty->setStyleSheet(QString("QLabel { color: %1; %2 background: transparent;"
                                      " font-style: italic; }")
                                  .arg(Theme::Color::textMuted)
                                  .arg(Theme::Type::qss(Theme::Type::Body)));
    ...
    m_indHashVal->setStyleSheet(QString("QLabel { color: %1; %2 background: transparent;"
                                        " font-family: monospace; }")
                                    .arg(Theme::Color::textPrimary)
                                    .arg(Theme::Type::qss(Theme::Type::Small)));
  ...
  connect(m_btnIgnore, &QPushButton::clicked, this, &AlertDetailPanel::onIgnoreClicked);
  ...
  connect(m_btnQuarantine, &QPushButton::clicked, this, &AlertDetailPanel::onQuarantineClicked);
  actions->addWidget(m_btnQuarantine);

  m_btnOpenLoc = new QPushButton("Open Location", this);
  styleSecondary(m_btnOpenLoc);
  connect(m_btnOpenLoc, &QPushButton::clicked, this, &AlertDetailPanel::onOpenLocationClicked);
  actions->addWidget(m_btnOpenLoc);

  m_btnCopy = new QPushButton("Copy Details", this);
  styleSecondary(m_btnCopy);
  connect(m_btnCopy, &QPushButton::clicked, this, &AlertDetailPanel::onCopyDetailsClicked);
...
void AlertDetailPanel::setSectionsVisible(bool v) {
  auto setVis = [v](QWidget* w) {
    if (w)
      w->setVisible(v);
  };
  setVis(m_severity);
  setVis(m_statusBadge);
  setVis(m_timestampLab);
  setVis(m_titleLab);
  setVis(m_categoryLab);
  setVis(m_tabs);
  setVis(m_btnInvestigate);
  setVis(m_btnIgnore);
  setVis(m_btnOpenLoc);
  setVis(m_btnCopy);
  // Quarantine visibility is decided per-alert in setAlert() when v=true,
  // so we only force-hide here when v=false.
  if (m_btnQuarantine && !v)
    m_btnQuarantine->setVisible(false);
  if (m_groupBanner)
    m_groupBanner->setVisible(false);
}
...
void AlertDetailPanel::setAlert(const EDR::Alert& alert, int groupCount) {
  ...
  if (m_btnQuarantine) {
    ...
    const bool sourceIsFile =
        !alert.sourcePath.isEmpty() && fi.isAbsolute() && fi.exists() && fi.isFile();
  ...
  m_timestampLab->setText(
      alert.lastSeen.isValid() ? alert.lastSeen.toString("MMM d, yyyy hh:mm:ss")
  ...
  if (m_statusBadge) {
    ...
    const QString hex = resolved ? Theme::Color::textMuted : Theme::Color::severitySafe;
    ...
    m_statusBadge->setToolTip(
        resolved && alert.resolvedAt.isValid()
            ? QString("Resolved %1").arg(alert.resolvedAt.toString("MMM d hh:mm:ss"))
  ...
  if (totalOccurrences > 1 && m_groupBanner) {
    ...
    if (alert.status == EDR::AlertStatus::Resolved) {
      msg = QString(
                "Condition observed across %1 monitoring tick%2 "
                "before resolving.")
                .arg(alert.ticksSeen)
                .arg(alert.ticksSeen == 1 ? "" : "s");
    } else if (alert.ticksSeen > 1) {
      msg = QString(
                "Persistent — observed across %1 monitoring ticks "
                "(%2 occurrences total).")
                .arg(alert.ticksSeen)
                .arg(totalOccurrences);
    } else {
      msg = QString(
                "This alert represents %1 occurrences within "
                "the current grouping window.")
                .arg(totalOccurrences);
    }
  ...
  // ── Overview ───────────────────────────────────────────────────────
  m_overSummary->setText(alert.description.isEmpty() ? "No summary captured." : alert.description);
  m_overFirstSeen->setText(
      alert.firstSeen.isValid() ? alert.firstSeen.toString("MMM d, yyyy hh:mm:ss")
                                : alert.timestamp.toString("MMM d, yyyy hh:mm:ss"));
  m_overLastSeen->setText(
      alert.lastSeen.isValid() ? alert.lastSeen.toString("MMM d, yyyy hh:mm:ss")
                               : alert.timestamp.toString("MMM d, yyyy hh:mm:ss"));
  m_overSource->setText(alert.sourcePath.isEmpty() ? "(unknown)" : alert.sourcePath);
  ...
  // ── Indicators ─────────────────────────────────────────────────────
  const bool hasTrust = (alert.signingStatus >= 0) || !alert.signerInfo.isEmpty();
  ...
  if (!hasTrust && !hasHash && !hasRep) {
    ...
    if (hasTrust) {
      ...
      m_indTrust->setText(QString("<span style='color:%1; font-weight:600;'>%2</span>")
...
void AlertDetailPanel::onOpenLocationClicked() {
  if (m_currentAlert.sourcePath.isEmpty())
    return;
  ...
  if (fi.exists()) {
    QDesktopServices::openUrl(
        QUrl::fromLocalFile(fi.isDir() ? fi.absoluteFilePath() : fi.absolutePath()));
  } else {
...
void AlertDetailPanel::onQuarantineClicked() {
  ...
  if (!fi.isFile()) {
    QMessageBox::warning(this, "Quarantine", "This alert does not point at a regular file.");
    return;
  }
  ...
  box.setInformativeText(
      QString("<b>%1</b><br><br><i>%2</i><br><br>"
              ...
              "action log.")
          .arg(fi.fileName().toHtmlEscaped(), m_currentAlert.sourcePath.toHtmlEscaped()));
  ...
  req.reason =
      std::string("Quarantine via Alerts page (alert: ") + m_currentAlert.title.toStdString() + ")";
  ...
  if (res.success) {
    ...
    QMessageBox::critical(this, "Quarantine failed", QString::fromStdString(err));
  }
}
...
void AlertDetailPanel::onIgnoreClicked() {
  ...
  req.userConfirmed = true;
  req.reason =
      std::string("Ignore via Alerts page (alert: ") + m_currentAlert.title.toStdString() + ")";
  ...
  if (cat == EDR::Category::Process) {
    ...
    req.target.path = m_currentAlert.sourcePath.toStdString();
    req.target.sha256 = m_currentAlert.sha256.toStdString();
    idLine = m_currentAlert.sha256.isEmpty() ? QString("Path: %1").arg(m_currentAlert.sourcePath)
    ...
    req.target.signatureKey = QString("%1::%2").arg(cat, m_currentAlert.sourcePath).toStdString();
    idLine = QString("Signature: %1::%2").arg(cat, m_currentAlert.sourcePath);
  }
  ...
  if (res.success) {
    ...
    const std::string err = res.errorMessage.empty() ? res.message : res.errorMessage;
    QMessageBox::critical(this, "Allowlist failed", QString::fromStdString(err));
  }
}
