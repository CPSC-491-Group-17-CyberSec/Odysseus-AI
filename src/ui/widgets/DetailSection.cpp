...
DetailSection::DetailSection(const QString& title,
                               const QString& accentHex,
                               QWidget*       parent)
    : QFrame(parent)
{
    m_accentHex = accentHex.isEmpty() ? QString(Theme::Color::accentBlue)
                                       : accentHex;
    ...
    setStyleSheet(
        "QFrame#OdyDetailSection { background: transparent; border: none; }");
    ...
    dot->setStyleSheet(QString(
        "QLabel { background-color: %1; border-radius: 3px; }")
            .arg(m_accentHex));
