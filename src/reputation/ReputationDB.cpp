...
QString severityToText(ReputationSeverity s) {
  switch (s) {
    case ReputationSeverity::Critical:
      return "critical";
    case ReputationSeverity::High:
      return "high";
    case ReputationSeverity::Medium:
      return "medium";
    case ReputationSeverity::Low:
      return "low";
    default:
      return "unknown";
  }
...
ReputationSeverity severityFromText(const QString& s) {
  ...
  if (t == "critical")
    return ReputationSeverity::Critical;
  if (t == "high")
    return ReputationSeverity::High;
  if (t == "medium")
    return ReputationSeverity::Medium;
  if (t == "low")
    return ReputationSeverity::Low;
...
bool ReputationDB::open(const QString& appDataDir, const QString& seedHashFile) {
  ...
  if (seedHashFile.isEmpty()) {
    ...
    if (n > 0) {
      ...
      qWarning().noquote() << "[Reputation] seed file" << seedHashFile
...
int ReputationDB::rowCount() const {
  ...
  sqlite3_stmt* stmt = nullptr;
  if (sqlite3_prepare_v2(m_db, "SELECT COUNT(*) FROM reputation;", -1, &stmt, nullptr) !=
      SQLITE_OK) {
    return 0;
  }
  int n = 0;
...
QHash<QString, QString> ReputationDB::snapshotHashIndex() const {
  ...
  while (sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    if (!h)
      continue;
    out.insert(
        QString::fromUtf8(reinterpret_cast<const char*>(h)),
        f ? QString::fromUtf8(reinterpret_cast<const char*>(f)) : QStringLiteral("Unknown"));
  }
  sqlite3_finalize(stmt);
  return out;
}
...
int ReputationDB::pruneAIUpserted() {
  ...
  if (deleted > 0)
    qInfo().noquote() << QString(
                             "[Reputation] Pruned %1 AI-auto-upserted row(s) "
...
bool ReputationDB::upsert(const ReputationRecord& r) {
  ...
  const QString firstIso = r.firstSeen.isValid() ? r.firstSeen.toString(Qt::ISODate) : nowIso;
  const QString lastIso = r.lastSeen.isValid() ? r.lastSeen.toString(Qt::ISODate) : nowIso;
...
bool ReputationDB::recordSighting(const QString& sha256) {
  ...
  const QByteArray nowIso = QDateTime::currentDateTimeUtc().toString(Qt::ISODate).toUtf8();
  const QByteArray hashLower = sha256.toLower().toUtf8();
...
int ReputationDB::importFromTextFile(const QString& path, const QString& source) {
  ...
  while (!in.atEnd()) {
    ...
    const QString family = (sp > 0) ? line.mid(sp + 1).trimmed() : QStringLiteral("Unknown");
  ...
  if (skippedBad > 0)
    qWarning().noquote() << QString(
                                "[Reputation] import: skipped %1 malformed line(s) "
                                "(non-64-char hash field)")
                                .arg(skippedBad);
  if (upsertFail > 0)
    qWarning().noquote() << QString(
                                "[Reputation] import: %1 row(s) failed to upsert "
                                "— last sqlite error: %2")
                                .arg(upsertFail)
