...
QString statusToText(Status s) {
  switch (s) {
    case Status::SignedTrusted:
      return "signed (trusted)";
    case Status::SignedUntrusted:
...
Result verifyFile(const QString& filePath) {
  ...
  const ToolResult t = runTool("/usr/bin/codesign", {"-dv", "--verbose=2", filePath});
  ...
  QRegularExpression authRx(R"(^Authority=(.+)$)", QRegularExpression::MultilineOption);
  QRegularExpressionMatch authM = authRx.match(combined);
  if (authM.hasMatch())
    out.signerId = authM.captured(1).trimmed();

  QRegularExpression teamRx(R"(^TeamIdentifier=([A-Z0-9]+)$)", QRegularExpression::MultilineOption);
...
bool checkRpmOwnership(const QString& filePath, QString& outPackage) {
  ...
  if (t.exitCode == 0 && !t.stdoutText.contains("not owned", Qt::CaseInsensitive) &&
...
Result verifyFile(const QString& /*filePath*/) {
  ...
  out.status = Status::Unknown;
  out.rawDetails =
      "Authenticode signature checks not implemented yet "
