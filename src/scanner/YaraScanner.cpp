...
QStringList getRuleTags(YR_RULE* rule)
{
    ...
    yr_rule_tags_foreach(rule, t) {
        if (t) tags.append(QString::fromUtf8(t));
    }
    return tags;
}
...
int scanCallback(YR_SCAN_CONTEXT* /*context*/,
                 int message,
                 void* messageData,
                 void* userData)
{
...
// ----------------------------------------------------------------------------
void yaraCompilerCallback(int errorLevel,
                          const char* fileName,
                          int lineNumber,
                          const YR_RULE* /*rule*/,
                          const char* message,
                          void* userData)
{
    ...
    const QString file = fileName ? QFileInfo(QString::fromUtf8(fileName)).fileName()
...
int compileRulesDir(const YaraInitOptions& opts, YR_RULES** outRules)
{
    ...
    if (!dir.exists()) {
        qWarning().noquote()
            << "[YARA] rules directory does not exist:" << opts.rulesDir
    ...
    if (!opts.includeExperimental && !opts.experimentalSubdir.isEmpty()) {
        experimentalAbs = QDir(opts.rulesDir)
                              .absoluteFilePath(opts.experimentalSubdir);
    }
    ...
    while (it.hasNext()) {
        ...
        if (!experimentalAbs.isEmpty()
            && fi.absoluteFilePath().startsWith(experimentalAbs))
        {
            ++skippedExperimental;
            continue;
        }
    ...
    if (skippedExperimental > 0) {
        qInfo().noquote()
            << QString("[YARA] skipped %1 experimental rule file(s) under %2 "
                       "(set experimentalRules=true in odysseus_config.json to enable)")
                .arg(skippedExperimental).arg(experimentalAbs);
    }

    if (files.isEmpty()) {
        qWarning().noquote()
            << "[YARA] no .yar/.yara files found under" << opts.rulesDir;
        yr_compiler_destroy(compiler);
        return 0;
    }
    ...
    for (const QFileInfo& fi : files) {
        if (totalErrors >= opts.maxCompileErrors) {
            qWarning().noquote()
                << QString("[YARA] hit maxCompileErrors=%1 — stopping compile pass "
        ...
        if (!f) {
            qWarning().noquote()
                << "[YARA] could not open rule file:" << fi.absoluteFilePath();
            continue;
        }
        const QByteArray nsBytes = fi.completeBaseName().toUtf8();
        const int errors = yr_compiler_add_file(compiler,
                                                f,
                                                nsBytes.constData(),
                                                pathBytes.constData());
        std::fclose(f);
        if (errors > 0) {
            qWarning().noquote()
                << QString("[YARA] %1 compile error(s) in %2 — rule(s) skipped"
                           "%3")
                    .arg(errors).arg(fi.fileName())
    ...
    yr_rules_foreach(compiled, rule) { (void)rule; ++n; }

    *outRules = compiled;
    qInfo().noquote()
        << QString("[YARA] compile pass complete: %1 rule(s) from %2 file(s) ok, "
...
bool initialize(const YaraInitOptions& opts)
{
    ...
    if (g_rules)                return true;   // partial state — treat as init done
    ...
    g_available = true;
    qInfo().noquote()
        << QString("[YARA] active with %1 rule(s) from %2%3")
              .arg(n)
              .arg(opts.rulesDir)
...
bool isAvailable() { return g_available && g_rules != nullptr; }
int  ruleCount()   { return g_ruleCount; }
...
YaraScanResult scanFile(const QString& filePath)
{
    ...
    const int rc = yr_rules_scan_file(g_rules,
                                      pathBytes.constData(),
                                      /*flags*/ 0,
                                      &scanCallback,
                                      &ctx,
