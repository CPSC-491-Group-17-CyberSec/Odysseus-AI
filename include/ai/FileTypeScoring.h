...
struct CalibrationCurve {
  ...
  float calibrate(float rawScore) const {
    if (points.size() < 2)
      return rawScore;  // fallback: identity
...
inline FileTypeProfile PEBinary() {
  ...
  p.calibration.points = {
      {0.0f, 0.0f}, {0.50f, 0.45f}, {0.70f, 0.65f}, {0.85f, 0.82f}, {1.0f, 1.0f}};
...
inline FileTypeProfile Script() {
  ...
  p.calibration.points = {
      {0.0f, 0.0f}, {0.55f, 0.30f}, {0.70f, 0.50f}, {0.85f, 0.75f}, {1.0f, 1.0f}};
...
inline FileTypeProfile TextData() {
  ...
  p.calibration.points = {
      {0.0f, 0.0f}, {0.65f, 0.05f}, {0.80f, 0.15f}, {0.90f, 0.40f}, {0.95f, 0.65f}, {1.0f, 1.0f}};
...
inline FileTypeProfile Archive() {
  ...
  p.calibration.points = {
      {0.0f, 0.0f}, {0.70f, 0.10f}, {0.85f, 0.30f}, {0.92f, 0.55f}, {0.97f, 0.80f}, {1.0f, 1.0f}};
...
inline FileTypeProfile Installer() {
  ...
  p.calibration.points = {
      {0.0f, 0.0f}, {0.65f, 0.10f}, {0.80f, 0.30f}, {0.90f, 0.55f}, {0.95f, 0.80f}, {1.0f, 1.0f}};
...
inline FileTypeProfile MediaBinary() {
  ...
  p.calibration.points = {
      {0.0f, 0.0f}, {0.70f, 0.05f}, {0.85f, 0.25f}, {0.93f, 0.55f}, {0.97f, 0.80f}, {1.0f, 1.0f}};
...
inline FileTypeProfile SourceCode() {
  ...
  p.calibration.points = {
      {0.0f, 0.0f}, {0.60f, 0.03f}, {0.80f, 0.10f}, {0.90f, 0.25f}, {0.95f, 0.50f}, {1.0f, 1.0f}};
...
inline FileTypeProfile CompiledArtifact() {
  ...
  p.calibration.points = {
      {0.0f, 0.0f}, {0.70f, 0.05f}, {0.85f, 0.15f}, {0.93f, 0.35f}, {0.97f, 0.60f}, {1.0f, 1.0f}};
...
inline FileTypeProfile BuildOutput() {
  ...
  p.calibration.points = {
      {0.0f, 0.0f}, {0.65f, 0.03f}, {0.80f, 0.08f}, {0.90f, 0.20f}, {0.95f, 0.45f}, {1.0f, 1.0f}};
...
inline FileTypeProfile Unknown() {
  ...
  p.calibration.points = {
      {0.0f, 0.0f}, {0.55f, 0.35f}, {0.75f, 0.60f}, {0.90f, 0.82f}, {1.0f, 1.0f}};
...
inline WeightedIndicatorResult computeWeightedIndicators(
    const std::vector<float>& features, const FileTypeProfile& profile) {
  ...
  if (w[1] > 0.01f) {
    ...
    if (features[1] > t.entropyStrong) {
      ...
      result.weakCount++;
      result.weakIndicators.push_back("Elevated entropy (" + fmt(features[1], 2) + "/8.0)");
    }
  }
  ...
  if (w[16] > 0.01f && features[16] > 0.5f) {
    ...
    if (features[20] < 0.5f && w[20] > 0.01f) {
      ...
      result.strongCount++;
      result.strongIndicators.push_back("Entry point outside code section");
    }
    if (features[25] > 0.5f && w[25] > 0.01f) {
      triggeredWeight += w[25];
      result.strongCount++;
      result.strongIndicators.push_back("Anomalous PE section names — possible packer");
    }
    if (features[27] > 0.5f && w[27] > 0.01f) {
      triggeredWeight += w[27];
      result.strongCount++;
      result.strongIndicators.push_back("Inflated virtual-to-raw size ratio");
    }
  }
  ...
  if (w[32] > 0.01f) {
    ...
    if (features[32] > t.suspiciousApiStrong) {
      ...
      result.weakCount++;
      result.weakIndicators.push_back("Some API references detected");
    }
  }
  ...
  if (w[33] > 0.01f) {
    ...
    if (features[33] > t.urlDensityStrong) {
      ...
      result.strongCount++;
      result.strongIndicators.push_back("High URL density — possible phishing or redirect chain");
  ...
  if (w[34] > 0.01f) {
    ...
    if (features[34] > t.ipAddressStrong) {
      ...
      result.strongCount++;
      result.strongIndicators.push_back("Multiple embedded IP addresses — possible C2 beaconing");
  ...
  if (w[36] > 0.01f) {
    ...
    if (features[36] > t.base64Strong) {
      ...
      result.strongCount++;
      result.strongIndicators.push_back("Large base64 payloads — possible obfuscated content");
...
inline ClassificationResult classifyFileCalibrated(
  ...
  if (cr.fileCategory == FileCategory::WebContent && indicators.strongCount == 0) {
  ...
  cr.severity =
      (finalScore >= profile.anomalousCeiling) ? SeverityLevel::Medium : SeverityLevel::Low;
...
inline std::string debugScoringPipeline(
  ...
  ss << "=== Scoring Pipeline Debug ===\n"
     ...
     << "Thresholds: Clean < " << profile.cleanCeiling << " < Anomalous < "
     << profile.anomalousCeiling << " < Suspicious < " << profile.suspiciousCeiling
     << " < Critical\n"
