...
inline const char* classificationToString(ClassificationLevel cl) {
  switch (cl) {
    case ClassificationLevel::Clean:
      return "Clean";
    case ClassificationLevel::Anomalous:
      return "Anomalous";
    case ClassificationLevel::Suspicious:
      return "Suspicious";
    case ClassificationLevel::Critical:
      return "CRITICAL";
  }
  return "Unknown";
}
...
inline const char* classificationToColor(ClassificationLevel cl) {
  switch (cl) {
    ...
    case ClassificationLevel::Critical:
      return "\033[91;1m";      // Bright red bold
  }
  return "\033[0m";
}
...
inline FileCategory categorizeExtension(const std::string& ext) {
  ...
  static const std::set<std::string> source = {
      "cpp",   "c", "cc", "cxx", "h",   "hpp",   "hxx", "hh",  "java", "cs",    "rs",     "go",
      "swift", "m", "mm", "kt",  "kts", "scala", "zig", "nim", "d",    "proto", "thrift", "fbs",
  ...
  static const std::set<std::string> web = {
      "html", "htm", "css", "svg", "xhtml", "jsp", "asp", "aspx", "vue", "jsx", "tsx", "svelte"};
  ...
  static const std::set<std::string> archives = {
      "zip", "gz", "bz2", "xz", "7z", "rar", "tar", "tgz", "tbz2", "zst", "lz4"};
  // Installers
  static const std::set<std::string> installers = {
      "dmg", "pkg", "deb", "rpm", "msi", "appimage", "snap", "flatpak", "iso", "img"};
  ...
  static const std::set<std::string> media = {
      ...
      "nib",
      "storyboardc",
      "car",  // Interface Builder + asset catalogs
      "ttf",
      "otf",
      "woff",
      "woff2",  // fonts
      ...
      "localstorage",
      "leveldb",  // Browser storage files
  };

  if (pe.count(ext))
    return FileCategory::PEBinary;
  if (source.count(ext))
    return FileCategory::SourceCode;
  if (compiled.count(ext))
    return FileCategory::CompiledArtifact;
  if (build.count(ext))
    return FileCategory::BuildOutput;
  if (scripts.count(ext))
    return FileCategory::Script;
  if (web.count(ext))
    return FileCategory::WebContent;
  if (text.count(ext))
    return FileCategory::TextData;
  if (archives.count(ext))
    return FileCategory::Archive;
  if (installers.count(ext))
    return FileCategory::Installer;
  if (media.count(ext))
    return FileCategory::MediaBinary;
...
inline FileCategory categorizeFilename(const std::string& filename) {
  static const std::set<std::string> buildFiles = {
      ...
      "cmakecache.txt",   "configure",         "configure.ac",
      "meson.build",      "build.gradle",      "build.ninja",
      ...
      "podfile",          "podfile.lock",      "conanfile.txt",
      "conanfile.py",     "vcpkg.json",        "meson_options.txt"};
...
inline const char* fileCategoryToString(FileCategory cat) {
  switch (cat) {
    case FileCategory::PEBinary:
      return "PE Binary";
    case FileCategory::Script:
      return "Script";
    case FileCategory::WebContent:
      return "Web Content";
    case FileCategory::TextData:
      return "Text/Data";
    case FileCategory::Archive:
      return "Archive";
    case FileCategory::Installer:
      return "Installer";
    case FileCategory::MediaBinary:
      return "Media/Document";
    case FileCategory::SourceCode:
      return "Source Code";
    case FileCategory::CompiledArtifact:
      return "Compiled Artifact";
    case FileCategory::BuildOutput:
      return "Build/Config";
...
inline float adjustedThreshold(float baseThreshold, FileCategory cat) {
  switch (cat) {
    case FileCategory::PEBinary:
      return baseThreshold;                           // calibrated
    case FileCategory::Script:
      return baseThreshold + 0.05f;                   // slight bump
    case FileCategory::WebContent:
      return std::min(baseThreshold + 0.30f, 0.95f);  // aggressive filter
    case FileCategory::TextData:
      return std::min(baseThreshold + 0.35f, 0.95f);  // very aggressive
    case FileCategory::Archive:
      return std::min(baseThreshold + 0.20f, 0.85f);
    case FileCategory::Installer:
      return std::min(baseThreshold + 0.25f, 0.90f);
    case FileCategory::MediaBinary:
      return std::min(baseThreshold + 0.25f, 0.90f);
    case FileCategory::SourceCode:
      return std::min(baseThreshold + 0.30f, 0.95f);  // devs files: high bar
    case FileCategory::CompiledArtifact:
      return std::min(baseThreshold + 0.30f, 0.95f);  // object files: high bar
    case FileCategory::BuildOutput:
      return std::min(baseThreshold + 0.35f, 0.95f);  // build configs: very high
    case FileCategory::Unknown:
      return baseThreshold + 0.10f;                   // moderate bump
  }
...
inline WebIndicatorAnalysis analyzeWebIndicators(const std::vector<float>& features) {
  ...
  if (features[1] > 7.2f) {
    ...
    result.strongIndicators.push_back(
        "Very high entropy for a text file — heavy obfuscation likely");
  }
  ...
  if (features[6] < 0.5f) {
    result.strongIndicatorCount++;
    result.strongIndicators.push_back(
        "Low printable ASCII ratio — unusual for web content, possible embedded binary");
  }
  ...
  if (features[32] > 0.2f) {
    result.strongIndicatorCount++;
    result.strongIndicators.push_back(
        "Contains system-level API references (CreateRemoteThread, VirtualAlloc, etc.)");
  }
  ...
  if (features[33] > 0.6f) {
    result.strongIndicatorCount++;
    result.strongIndicators.push_back(
        "Unusually high URL density — possible phishing or redirect chain");
  }
  ...
  if (features[34] > 0.2f) {
    result.strongIndicatorCount++;
    result.strongIndicators.push_back(
        "Multiple embedded IP addresses — possible C2 beaconing in web content");
  }
  ...
  if (features[35] > 0.0f) {
    result.strongIndicatorCount++;
    result.strongIndicators.push_back(
        "Windows registry paths in web content — likely ActiveX/exploit payload");
  }
  ...
  if (features[7] > 0.15f) {
    result.strongIndicatorCount++;
    result.strongIndicators.push_back(
        "Non-ASCII byte content in web file — possible embedded binary payload");
  }
...
inline int countStrongIndicators(const std::vector<float>& features) {
  ...
  if (features[16] > 0.5f) {
    if (features[18] > 0.875f)
      count++;  // high section entropy
    if (features[20] < 0.5f)
      count++;  // EP outside code
    if (features[25] > 0.5f)
      count++;  // anomalous section names
    if (features[27] > 0.5f)
      count++;  // inflated virtual size
  }
...
inline ClassificationResult classifyFile(
  ...
  if (cr.fileCategory == FileCategory::Archive || cr.fileCategory == FileCategory::Installer ||
...
inline std::vector<std::string> extractKeyIndicators(
  ...
  if (fileCategory == FileCategory::WebContent) {
    ...
    for (const auto& si : wa.strongIndicators) {
      if (static_cast<int>(result.size()) >= maxIndicators)
        break;
      result.push_back(si);
    }
  ...
  if (features[1] > 7.0f)
    ...
    candidates.push_back(
        {features[1] / 10.0f,
         "Elevated entropy (" + fmt(features[1], 2) + "/8.0) — moderate obfuscation signal"});
  ...
  if (features[16] > 0.5f) {
    if (features[18] > 0.875f)
      candidates.push_back(
          {features[18], "PE section with very high entropy — likely packed/encrypted code"});
    if (features[20] < 0.5f)
      candidates.push_back(
          {0.8f, "Entry point outside code section — unusual for legitimate software"});
    if (features[21] < 0.5f)
      candidates.push_back({0.6f, "No debug information — stripped binary, common in malware"});
    if (features[25] > 0.5f)
      candidates.push_back(
          {0.7f, "Anomalous PE section names — possible packer (UPX, Themida, etc.)"});
    if (features[27] > 0.5f)
      candidates.push_back(
          {features[27], "Inflated virtual-to-raw size ratio — unpacking indicator"});
  }
  ...
  if (features[32] > 0.3f)
    ...
    candidates.push_back({features[32] * 1.5f, "Some suspicious API references detected"});

  // Network indicators
  if (features[33] > 0.0f || features[34] > 0.0f)
    candidates.push_back(
        {std::max(features[33], features[34]),
         "Embedded URLs or IP addresses found — possible C2 communication"});

  // Registry paths
  if (features[35] > 0.0f)
    candidates.push_back(
        {features[35], "Registry path references — potential persistence mechanism"});

  // Base64 strings
  if (features[36] > 0.0f)
    candidates.push_back(
        {features[36], "Base64-encoded strings detected — possible payload obfuscation"});

  // Sort by suspicion score and take top N
  std::sort(candidates.begin(), candidates.end(), [](const Indicator& a, const Indicator& b) {
...
inline std::string formatTerminalOutput(const ScanResult& result) {
  ...
  ss << "  " << clColor << BOLD << clLabel << RESET << "  " << DIM << "Engine:" << RESET
  ...
  ss << "  " << CYAN << "Score:     " << RESET << clColor << std::fixed << std::setprecision(3)
     << result.anomalyScore << RESET << " / 1.000\n";
  ss << "  " << CYAN << "Threshold: " << RESET << std::setprecision(3) << result.threshold;
  if (std::abs(result.effectiveThreshold - result.threshold) > 0.001f) {
    ss << DIM << " (effective: " << result.effectiveThreshold << " for ." << result.fileExtension
       << ")" << RESET;
  }
  ss << "\n";
  ss << "  " << CYAN << "Verdict:   " << RESET << clColor << BOLD
     << classificationToString(result.classification) << RESET << "\n";
  ...
  if (!result.recommendedActions.empty()) {
    ...
    for (size_t i = 0; i < result.recommendedActions.size(); ++i) {
      ss << "    " << GREEN << (i + 1) << "." << RESET << " " << result.recommendedActions[i]
         << "\n";
    }
  }
  ...
  while (std::getline(stream, line)) {
    ...
    switch (current) {
      ...
      case Section::Indicators: {
        ...
        if (!cleaned.empty() && (cleaned[0] == '-' || cleaned[0] == '*')) {
          ...
          if (s != std::string::npos)
            cleaned = cleaned.substr(s);
        }
        if (!cleaned.empty())
          parsed.indicators.push_back(cleaned);
        break;
      }
      case Section::Actions: {
        ...
        if (!cleaned.empty())
          parsed.actions.push_back(cleaned);
        break;
      }
      default:
        if (!line.empty()) {
