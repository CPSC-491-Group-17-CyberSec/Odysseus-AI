...
class Quarantine {
 public:
  ...
  std::optional<QuarantineEntry> quarantine(
  ...
  ActionResult restore(const std::string& entryId, RestoreConflictPolicy policy);

  // Read-only views.
  std::vector<QuarantineEntry> list() const;
  std::optional<QuarantineEntry> findById(const std::string& id) const;
  std::optional<QuarantineEntry> findByOriginalPath(const std::string& originalPath) const;
